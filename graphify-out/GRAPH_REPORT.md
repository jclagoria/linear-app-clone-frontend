# Graph Report - linear-app-clone-frontend  (2026-07-30)

## Corpus Check
- 644 files · ~371,956 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6068 nodes · 6207 edges · 547 communities (493 shown, 54 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 20 edges (avg confidence: 0.67)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9f862333`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- errors/index.ts
- dependencies
- shared/ui/index.ts
- Requirement: ErrorTaxonomy
- Behaviour
- Requirement: ErrorTaxonomy
- Behaviour
- Behaviour
- Module Contracts & Interactions
- Flow Inventory
- Flow Inventory
- Auth Module — Frontend Specification
- Auth Module — Frontend Specification
- MSW Handlers — Frontend Specification
- MSW Handlers — Frontend Specification
- compilerOptions
- Auth — Frontend Specification
- Flow Inventory
- Sidebar/index.ts
- ADR-0006: Stack Selection — Zustand, Vite, React Router, Vitest
- UI Store — Frontend Specification
- UI Store — Frontend Specification
- compilerOptions
- ADR-0009: UI Component Architecture — Presentational Primitives
- Flow Inventory
- Cache Layer — Frontend Specification
- Issues Store — Frontend Specification
- Selectors — Frontend Specification
- Flow Inventory
- Cache Layer — Frontend Specification
- Requirement: IssuesStore
- Selectors — Frontend Specification
- Containerized SPA Serving — Frontend Specification
- Containerized SPA Serving — Frontend Specification
- Store Architecture — Frontend Specification
- Wireframe: {ScreenName}
- Store Architecture — Frontend Specification
- MSW Handlers — Frontend Design
- Browser Mode: Launch Dev App with Mocked API
- Tasks — State Module: Cache & Selectors (Frontend)
- API Client: Error Handling Flow
- Component Catalog
- Behaviour
- Component Catalog
- Available Features
- stores/selectors/index.ts
- Routing — Frontend Design
- WebSocket Store — Frontend Specification
- WebSocket Store — Frontend Specification
- ADR-0008: API Error Taxonomy via Class Hierarchy
- Frontend Architecture — Linear App Clone
- Component Catalog
- Auth Store — Frontend Specification
- Architecture
- Auth Store — Frontend Specification
- Header/index.ts
- ADR-0004: Feature-Sliced Design Architecture
- ADR-0005: React Hook Form + Zod for Form Management
- Component Catalog
- Component Catalog
- Auth — Frontend Design
- session/model/store.ts
- ADR-0007: Store Isolation — Separate Zustand Stores per Domain
- UI Generic Components — Frontend Design
- {Domain} — Frontend Specification
- Atomic Parts
- Auth Module — Frontend Design
- Component Catalog
- Tasks — Remove localStorage Token Storage, Use HttpOnly Cookie (Frontend)
- Container Build & Run: DockerBuildFlow
- Tasks — API Client & Error Handling (Frontend)
- Components
- Deployment
- Components
- Deployment — Linear App Clone Frontend
- Atomic Parts
- Atomic Parts
- State Module: Cache & Selectors — Frontend Design
- Tasks — Add Dockerfile for Containerization (Frontend)
- API Client & Error Handling — Frontend Design
- Flow Inventory
- ADR-0001: Zustand Issues Store Following Entity-Store Pattern
- OpenSpec Schema Collection
- {Domain} — Frontend Design
- devDependencies
- Atomic Parts
- Atomic Parts
- Atomic Parts
- Frontend Stack — Linear App Clone
- Tasks — Routing Module (Frontend)
- Design System — Linear App Clone (Frontend)
- Behaviour
- Tasks — UI Generic Components (Frontend)
- Tasks — {Change Title} (Frontend)
- {Domain}: {FlowName}
- Behaviour
- frontend/README.md
- Atomic Parts
- Atomic Parts
- Atomic Parts
- ADR Review Manifest
- Tasks — Auth Module (Frontend)
- Containerization — Frontend Design
- Tech Selection — Add Dockerfile for Containerization
- Layout Module — Frontend Design
- Design System — {Project Name}
- Mockups — {Domain}
- Review — Auth Module
- Tech Selection — Linear App Clone (Frontend)
- Review — Routing Module
- Tech Selection — Linear App Clone (Frontend)
- Tasks — Centralize MSW Handlers (Frontend)
- Tech Selection — Linear App Clone (Frontend)
- Tech Selection — Linear App Clone (Frontend)
- Tech Selection — Linear App Clone (Frontend)
- Tech Selection — Linear App Clone (Frontend)
- Tasks — Layout Module Page Structure & Theme (Frontend)
- Tech Selection — {Project Name} (Frontend)
- Stack
- Screen: {ScreenName}
- issue/api/index.ts
- handlers.ts
- 3. Work Module
- Auth Module — Login & Token Management
- Routing Module — Navigation & Route Matching
- Centralize MSW Handlers
- Review — Centralize MSW Handlers
- Tech Selection — Linear App Clone (Frontend)
- Remove localStorage Token Storage, Use HttpOnly Cookie via credentials: 'include'
- Review — Remove localStorage Token Storage, Use HttpOnly Cookie via credentials: 'include'
- State Module — Cache & Selectors
- Review — State Module: Cache & Selectors
- Design System — Add Dockerfile for Containerization
- Add Dockerfile for Containerization
- Review — Add Dockerfile for Containerization
- API Client & Error Handling Module
- Review — API Client & Error Handling
- UI Module — Generic Components
- Review — UI Generic Components
- UI Module — Frontend Specification
- Review — Layout Module Page Structure & Theme
- {Change Title}
- Review — {Change Title}
- UI Module — Frontend Specification
- main.tsx
- useLoginForm.ts
- cacheStore.ts
- websocketStore.ts
- ADR Review Manifest
- ADR Review Manifest
- ADR Review Manifest
- ADR Review Manifest
- ADR Review Manifest
- ADR Review Manifest
- ADR Review Manifest
- Requirement: Button Variants
- Requirement: Modal with Stack Management
- Tech Selection — Linear App Clone (Frontend)
- ADR Review Manifest
- Requirement: Button Variants
- Requirement: Modal with Stack Management
- toastStore.ts
- uiStore.ts
- ErrorBanner.tsx
- Requirement: Input Field
- Requirement: Toast Notifications
- Technology Templates — Frontend Schema
- Requirement: Input Field
- Requirement: Toast Notifications
- modalStore.ts
- stores/rate-limit.ts
- Breadcrumb.tsx
- AGENTS.md
- codegraph
- Design System — Linear App Clone
- Requirement: Loading Indicator
- Requirement: Select Dropdown
- Requirement: Textarea
- Requirement: Loading Indicator
- Requirement: Select Dropdown
- Requirement: Textarea
- useTheme.ts
- RateLimitToast.tsx
- vite-env.d.ts
- tsconfig.json
- Issue Labels — Frontend Specification
- eslint
- eslint-config-prettier
- eslint-import-resolver-typescript
- @eslint/js
- Issue Labels — Frontend Specification
- eslint-plugin-react
- form/ui/index.ts
- Behaviour
- jsdom
- lint-staged
- Flow Inventory
- Behaviour
- @playwright/test
- Component Catalog
- @testing-library/jest-dom
- Flow Inventory
- @testing-library/user-event
- Label Management — Frontend Design
- form/model/index.ts
- useKeyboardStore.ts
- typescript-eslint
- Comment Deletion — Frontend Specification
- Keyboard Module — Frontend Design
- Edit Comment — Frontend Specification
- router.tsx
- LoginForm.tsx
- ProjectsPage.tsx
- setup.ts
- README.md
- Layout Module — Page Structure & Theme
- ADR Review Manifest
- Behaviour
- Flow Inventory
- Component Catalog
- Tasks — Work Module: Issues Store & UI (Frontend)
- Work Module: Issues Store & UI — Frontend Design
- Review — Work Module: Issues Store & UI
- Tech Selection — Linear App Clone (Frontend)
- Work Module: Issues Store & UI Components
- ConfirmDeleteDialog.tsx
- Authentication — Frontend Specification
- Dedicated Status Transition Endpoint for Issue Workflow
- MockWSEndpoint
- IssueDetail.tsx
- Issues — Frontend Specification
- MockWebSocket
- Issue Status — Frontend Specification
- Issue Status — Frontend Specification
- Issue Assign/Unassign — Frontend Design
- Edit Comment — Frontend Specification
- Issue Status Transition — Frontend Design
- Component Catalog
- Tasks — Implement Issue Status Transitions (Frontend)
- Issue Detail: Change Issue Status
- Issue Assign — Frontend Specification
- Issue Assign — Frontend Specification
- Tech Selection — Linear App Clone (Frontend)
- Implement Issue Status Transitions
- Review — Implement Issue Status Transitions
- Dedicated Assignee Transition Endpoint for Issue Assignment
- ADR Review Manifest
- Issue Management: Change Assignee
- Design System — Linear App Clone Frontend
- Tasks — Issue Assign/Unassign Endpoint (Frontend)
- Review — Issue Assign/Unassign Endpoint
- Implement PATCH /issues/{id}/assignee Endpoint (Assign/Unassign)
- Tech Selection — Linear App Clone (Frontend)
- ADR Review Manifest
- Issue Watchers — Frontend Specification
- ADR-0012: Watcher Feature Slice — New FSD Entity and Feature for Issue Watchers
- WatcherList.tsx
- Label
- Issue Watchers — Frontend Specification
- Issue Watchers — Frontend Design
- Tasks — Issue Watchers (Frontend)
- Design System — Linear App Clone
- Issue Detail: Watch/Unwatch Issue
- Tech Selection — Linear App Clone (Frontend)
- Issue Watchers
- WatcherSection.tsx
- Review — Issue Watchers
- watchers/model/store.ts
- ADR Review Manifest
- Tasks — Label Management Endpoints for Issues (Frontend)
- Comment Deletion — Frontend Specification
- Tech Selection — Linear App Clone (Frontend)
- Label Management Endpoints for Issues
- Review — Label Management Endpoints for Issues
- ADR Review Manifest
- Edit Comment — Frontend Design
- Component Catalog
- Authentication — Frontend Specification
- label/model/validation.ts
- Tasks — Edit Comment (Frontend)
- Comments: Edit Own Comment
- Comment Deletion — Frontend Design
- Tasks — Delete Comment (Frontend)
- Tech Selection — Linear App Clone (Frontend)
- Issue Comments: Delete Comment
- Frontend Design — Register User API Integration
- Component Catalog
- Edit Comment
- Review — Edit Comment
- Tech Selection — Linear App Clone (Frontend)
- Delete Comment on Issue
- ADR Review Manifest
- Review — Delete Comment
- Issues Module — Pagination Response Shape — Frontend Specification
- Issues — Frontend Design
- Behaviour
- Issues: Browse and Paginate Issues
- Issues Module — Pagination Response Shape — Frontend Specification
- Components
- Requirement: IssuesFilters
- ADR Review Manifest
- Fix: API Client Inconsistency — Store Uses Raw Fetch Instead of apiClient
- Review — Fix: API Client Inconsistency
- Issues Module — Frontend Specification
- ADR Review Manifest
- Tasks — Fix: API Client Inconsistency (Frontend)
- Requirement: IssueForm
- Issue Model — Frontend Specification
- Requirement: IssueDetail
- Requirement: IssueList
- Tech Selection — Fix: API Client Inconsistency
- Requirement: LoadNextPageUsesApiClient
- 2026-07-23-fix-api-client-inconsistency/design-system.md
- Issues: Browse Paginated List
- Issues Module — Frontend Design (Pagination Shape Fix)
- Fix: Pagination response shape mismatch — meta vs pagination
- Review — Fix Pagination Response Shape
- Tasks — Fix Pagination Response Shape (Frontend)
- ADR Review Manifest
- Tech Selection — Fix Pagination Response Shape
- Issue API — Frontend Specification
- Issue API — Frontend Specification
- Issue API — Frontend Design
- Tasks — Fix Delete Issue Response Type (Frontend)
- Issue Management: Delete Issue
- Requirement: Validation Types
- Tech Selection — Fix Delete Issue Response Type
- Issues Filtering — Frontend Specification
- Issues Filtering — Frontend Specification
- Issues Filtering — Frontend Design
- Tasks — Fix Query Parameter Mismatch (Frontend)
- Issues: Filter Issues List
- Fix: Query Parameter Mismatch — status vs statusId, extra search/priority
- Review — Fix Query Parameter Mismatch
- ADR Review Manifest
- Tech Selection — Fix Query Parameter Mismatch
- 2026-07-23-fix-query-param-mismatch/design-system.md
- Design System — Fix Pagination Response Shape
- Issue Creation — Frontend Specification
- Issue Creation — Frontend Specification
- Issue Creation — Frontend Design
- Design System — Linear App Clone
- Issue Creation: Create Issue with Team Context
- Tasks — Fix: Create Issue Missing Required teamId Field (Frontend)
- Tech Selection — Linear App Clone (Frontend)
- Fix: Create Issue Missing Required teamId Field
- Review — Fix: Create Issue Missing Required teamId Field
- team/model/store.ts
- ADR Review Manifest
- Design System — Fix Delete Issue Response Type
- Fix: Delete Issue Response Type Mismatch
- Review — Fix Delete Issue Response Type
- ADR Review Manifest
- Issue Model — Frontend Specification
- 2026-07-23-fix-delete-issue-response-type/mockups/README.md
- Issue Model Alignment — Frontend Design
- Issue Model: Type Alignment
- Tasks — Fix: Issue Model Missing 7 Fields from API Spec (Frontend)
- Tech Selection — Fix: Issue Model Missing Fields (Frontend)
- Design System — Fix: Issue Model Missing Fields
- Fix: Issue Model Missing 7 Fields from API Spec
- Review — Fix: Issue Model Missing 7 Fields from API Spec
- Requirement: Validation Types
- Behaviour
- msw
- @tailwindcss/vite
- Behaviour
- Behaviour
- vitest
- ADR Review Manifest
- Behaviour
- Form Module — Frontend Design
- Component Catalog
- Tech Selection — Form Module (Validation & Submission)
- EditIssuePage.tsx
- CreateIssuePage.tsx
- Behaviour
- Tasks — Form Module: Validation & Submission (Frontend)
- Review — Form Module: Validation & Submission
- Form Module — Validation & Submission
- ADR Review Manifest
- Realtime Module — Event Subscription & Processing
- Realtime Module — Event Subscription & Processing
- IssueForm.tsx
- Flow Inventory
- Behaviour
- Behaviour
- realtime/lib/index.ts
- Realtime Module — WebSocket Connection Management
- Create Project — Frontend Specification
- Behaviour
- event-schema.ts
- Realtime Module — WebSocket Connection Management
- Realtime Module — Frontend Design
- Component Catalog
- Architecture — Realtime Module (Frontend)
- Deployment — Realtime Module (Frontend)
- Review — Realtime WebSocket Events
- Realtime Module — WebSocket & Events
- ADR Review Manifest — Realtime WebSocket Events
- Tasks — Realtime WebSocket Events (Frontend)
- OptimisticProvider.tsx
- WebSocketProvider.tsx
- Project Summary — Realtime WebSocket Events
- Stack — Realtime Module (Frontend)
- Tech Selection — Realtime Module (Frontend)
- Behaviour
- Flow Inventory
- ADR-001: Context-Specificity for Keyboard Shortcut Conflict Resolution
- Review — Keyboard Module (LAG-29)
- ADR-002: Zustand + localStorage for Offline-First Shortcut Customizations
- ADR-003: Server-Sent Events over WebSocket for Real-Time Updates
- event-processor.ts
- KeyboardProvider.tsx
- Issue
- Component Catalog
- api/shortcuts.ts
- keyboard/ui/ToastContainer.tsx
- DeleteConfirmModal.tsx
- shortcutActions.ts
- KeyboardShortcutWrapper.tsx
- Tasks — Keyboard Module (Frontend)
- Tech Selection — Linear App Clone (Frontend)
- Ticket 10: Keyboard Module — Shortcuts & Context
- Component Catalog
- Behaviour
- Flow Inventory
- ADR-0013: SSE for Real-Time Transport
- ADR-0014: Optimistic Updates with Rollback
- Component Catalog
- Real-time Issue Updates — Frontend Design
- Tasks — Real-time Issue Updates (Frontend)
- Tech Selection — Real-time Issue Updates (Frontend)
- Review — Real-time Issue Updates
- Tech Selection — Linear App Clone (Frontend)
- Real-time Issue Updates
- ADR Review Manifest
- issue/model/store.ts
- event-router.ts
- MockWebSocket
- Layout Module — Frontend Specification
- notificationsStore.ts
- Layout Module: Dashboard View
- Authentication: Login
- useIssueBoard.ts
- Review — Layout Module — Responsive & Theme Finalization
- Layout Module — Frontend Design
- Component Catalog
- Tasks — Layout Module — Responsive & Theme Finalization (Frontend)
- ADR Review Manifest
- Tech Selection — Linear App Clone (Frontend)
- Layout Module — Responsive & Theme Finalization
- ThemeSelector.tsx
- realtime/ui/ToastContainer.tsx
- NotificationItem.tsx
- CommentItem.tsx
- useNotifications.ts
- realtime/ui/EmptyState.tsx
- CommentThreadPage.tsx
- IssueBoardPage.tsx
- List Projects — Frontend Specification
- Behaviour
- New Durable ADRs Created
- Frontend WebSocket Changes — Align with AsyncAPI Spec
- WebSocket Frontend Alignment — Frontend Design
- Flow Inventory
- MockWebSocket
- package.json
- scripts
- Component Catalog
- Review — WebSocket Frontend Alignment
- Tasks — WebSocket Frontend Alignment (Frontend)
- Tech Selection — Linear App Clone (Frontend)
- WebSocket Frontend: Align Client with AsyncAPI Specification
- Requirement: LoadTeamsOnAuth
- @commitlint/config-conventional
- List Projects — Frontend Specification
- api/auth.ts
- Tasks — Register User API Integration (Frontend)
- useRegisterForm.ts
- RTK (Rust Token Killer) - Token-Optimized Commands
- RegisterForm.tsx
- RegisterPage.tsx
- Tech Selection — Linear App Clone (Frontend)
- Comment
- Implement Register User API Integration
- Review — Register User API Integration
- ADR Review Manifest
- realtime/ui/index.ts
- CreateProjectDialog.tsx
- Component Catalog
- List Projects — Frontend Design
- Tasks — List Projects API (Frontend)
- Projects: List Projects
- Tech Selection — Linear App Clone (Frontend)
- List Projects API
- Create Project — Frontend Specification
- Component Catalog
- projects.ts
- ProjectsPage.test.tsx
- Review — List Projects API (Frontend)
- ADR Review Manifest
- Component Catalog
- MockWSEndpoint
- Create Project API — Frontend Design
- Tasks — Create Project API (Frontend)
- Projects: Create Project
- Tech Selection — Create Project API (Frontend)
- Create Project API
- Review — Create Project API
- ADR Review Manifest
- create-project-api.test.ts
- Org Sidebar — Frontend Design
- Tasks — Consume Teams API and Render Org Sidebar (Frontend)
- Navigation: Org Sidebar
- Tech Selection — Consume Teams API and Render Org Sidebar (Frontend)
- Consume Teams API and Render Org Sidebar
- Review — Consume Teams API and Render Org Sidebar
- ADR Review Manifest

## God Nodes (most connected - your core abstractions)
1. `ApiError` - 25 edges
2. `processEvent()` - 20 edges
3. `compilerOptions` - 19 edges
4. `routeEvent()` - 17 edges
5. `Issue` - 16 edges
6. `setupEventRouter()` - 16 edges
7. `useOptimisticStore` - 15 edges
8. `compilerOptions` - 15 edges
9. `Tasks — State Module: Cache & Selectors (Frontend)` - 15 edges
10. `ApiClient` - 14 edges

## Surprising Connections (you probably didn't know these)
- `FetchIssuesResponse` --references--> `Issue`  [EXTRACTED]
  linear-frontend-v1/src/entities/issue/api/index.ts → linear-frontend-v1/src/entities/issue/model/types.ts
- `IssuesState` --references--> `Comment`  [EXTRACTED]
  linear-frontend-v1/src/entities/issue/model/store.ts → linear-frontend-v1/src/entities/issue/model/types.ts
- `IssuesState` --references--> `Issue`  [EXTRACTED]
  linear-frontend-v1/src/entities/issue/model/store.ts → linear-frontend-v1/src/entities/issue/model/types.ts
- `IssueCardProps` --references--> `Issue`  [EXTRACTED]
  linear-frontend-v1/src/entities/issue/ui/IssueCard.tsx → linear-frontend-v1/src/entities/issue/model/types.ts
- `IssueDetailProps` --references--> `Issue`  [EXTRACTED]
  linear-frontend-v1/src/entities/issue/ui/IssueDetail.tsx → linear-frontend-v1/src/entities/issue/model/types.ts

## Import Cycles
- None detected.

## Communities (547 total, 54 thin omitted)

### Community 0 - "errors/index.ts"
Cohesion: 0.07
Nodes (36): ApiClient, ApiClientConfig, RequestInterceptor, RequestMethod, RequestOptions, ResponseInterceptor, dispatchTable, dispatchToHandler() (+28 more)

### Community 1 - "dependencies"
Cohesion: 0.09
Nodes (23): class-variance-authority, clsx, @hookform/resolvers, dependencies, class-variance-authority, clsx, @hookform/resolvers, lucide-react (+15 more)

### Community 2 - "shared/ui/index.ts"
Cohesion: 0.06
Nodes (32): Button, ButtonProps, ButtonSize, ButtonVariant, sizeStyles, variantStyles, Card(), CardProps (+24 more)

### Community 3 - "Requirement: ErrorTaxonomy"
Cohesion: 0.05
Nodes (40): Accessibility, API Client — Frontend Specification, ApiClient, ApiError (base), Behaviour, Components, Error Handling Flow, ErrorHandler (+32 more)

### Community 4 - "Behaviour"
Cohesion: 0.05
Nodes (40): Accessibility, Behaviour, Components, Header, Layout Module — Frontend Specification, PageLayout, Requirement: PageLayoutStructure, Requirement: ResponsiveBreakpoints (+32 more)

### Community 5 - "Requirement: ErrorTaxonomy"
Cohesion: 0.05
Nodes (40): Accessibility, API Client — Frontend Specification, ApiClient, ApiError (base), Behaviour, Components, Error Handling Flow, ErrorHandler (+32 more)

### Community 6 - "Behaviour"
Cohesion: 0.05
Nodes (38): Accessibility, Behaviour, Components, NotFoundPage, Requirement: BrowserHistory, Requirement: ProgrammaticNavigation, Requirement: QueryParameters, Requirement: RouteGuards (+30 more)

### Community 7 - "Behaviour"
Cohesion: 0.05
Nodes (38): Accessibility, Behaviour, Components, NotFoundPage, Requirement: BrowserHistory, Requirement: ProgrammaticNavigation, Requirement: QueryParameters, Requirement: RouteGuards (+30 more)

### Community 8 - "Module Contracts & Interactions"
Cohesion: 0.06
Nodes (33): 10. Module Dependency Matrix, 1. Communication Overview, 2.1 Request Format, 2.2 Response Format, 2. API Contract, 3.1 Login Request/Response, 3.2 Token Refresh Contract, 3.3 Authenticated Request (+25 more)

### Community 9 - "Flow Inventory"
Cohesion: 0.06
Nodes (31): Actors, Flow Inventory, Navigation Graph, Navigation Graph, Navigation Graph, Navigation Graph, Navigation Graph, Navigation Graph (+23 more)

### Community 10 - "Flow Inventory"
Cohesion: 0.06
Nodes (31): Actors, Auth: Login Flow, Flow Inventory, Issues: Browse & Filter, Issues: Create / Update, Navigation Graph, Navigation Graph, Navigation Graph (+23 more)

### Community 11 - "Auth Module — Frontend Specification"
Cohesion: 0.07
Nodes (26): Accessibility, Auth Module — Frontend Specification, AuthGuard, Behaviour, Components, LoginForm, Requirement: LoginFlow, Requirement: Logout (+18 more)

### Community 12 - "Auth Module — Frontend Specification"
Cohesion: 0.07
Nodes (26): Accessibility, Auth Module — Frontend Specification, AuthGuard, Behaviour, Components, LoginForm, Requirement: LoginFlow, Requirement: Logout (+18 more)

### Community 13 - "MSW Handlers — Frontend Specification"
Cohesion: 0.08
Nodes (25): Accessibility, Behaviour, `browser.ts`, Components, `handlers.ts`, MSW Handlers — Frontend Specification, Requirement: AuthHandlers, Requirement: BrowserRuntime (+17 more)

### Community 14 - "MSW Handlers — Frontend Specification"
Cohesion: 0.08
Nodes (25): Accessibility, Behaviour, `browser.ts`, Components, `handlers.ts`, MSW Handlers — Frontend Specification, Requirement: AuthHandlers, Requirement: BrowserRuntime (+17 more)

### Community 15 - "compilerOptions"
Cohesion: 0.08
Nodes (25): compilerOptions, allowImportingTsExtensions, baseUrl, isolatedModules, jsx, lib, module, moduleDetection (+17 more)

### Community 16 - "Auth — Frontend Specification"
Cohesion: 0.08
Nodes (23): Accessibility, Auth — Frontend Specification, Auth types (modified), Behaviour, Components, Requirement: LoginStoresCookie, Requirement: LogoutClearsCookie, Requirement: MSWHandlersReflectNewContract (+15 more)

### Community 17 - "Flow Inventory"
Cohesion: 0.08
Nodes (23): Actors, Auth: Login, Auth: Logout, Auth: Session Expiry (Passive), Auth: Session Hydrate (Page Refresh), Auth: Silent Token Refresh, Flow Inventory, Navigation Graph (+15 more)

### Community 18 - "Sidebar/index.ts"
Cohesion: 0.14
Nodes (16): useActiveRoute(), HamburgerButton, HamburgerButtonProps, MobileSidebarOverlay(), MobileSidebarOverlayProps, navItems, teams, NavLink (+8 more)

### Community 19 - "ADR-0006: Stack Selection — Zustand, Vite, React Router, Vitest"
Cohesion: 0.09
Nodes (21): ADR-0006: Stack Selection — Zustand, Vite, React Router, Vitest, Build Tool: Vite vs Webpack vs Turbopack, Chosen: React Router DOM (routing), Chosen: Vite (build tool), Chosen: Vitest (test runner), Chosen: Zustand (state management), Confirmation, Consequences (+13 more)

### Community 20 - "UI Store — Frontend Specification"
Cohesion: 0.10
Nodes (20): Accessibility, Behaviour, Components, Requirement: KeyboardContext, Requirement: Modals, Requirement: Sidebar, Requirement: Theme, Routing (+12 more)

### Community 21 - "UI Store — Frontend Specification"
Cohesion: 0.10
Nodes (20): Accessibility, Behaviour, Components, Requirement: KeyboardContext, Requirement: Modals, Requirement: Sidebar, Requirement: Theme, Routing (+12 more)

### Community 22 - "compilerOptions"
Cohesion: 0.10
Nodes (20): compilerOptions, allowImportingTsExtensions, erasableSyntaxOnly, lib, module, moduleDetection, noEmit, noFallthroughCasesInSwitch (+12 more)

### Community 23 - "ADR-0009: UI Component Architecture — Presentational Primitives"
Cohesion: 0.10
Nodes (19): ADR-0009: UI Component Architecture — Presentational Primitives, Chosen: class-variance-authority for variant management, Chosen: component-local useState for field primitives, Chosen: forwardRef + native HTML attribute extension, Chosen: Zustand stores for cross-cutting UI state, Component API: forwardRef + native props vs custom prop interface, Component-local useState, Confirmation (+11 more)

### Community 24 - "Flow Inventory"
Cohesion: 0.10
Nodes (19): Actors, Auth: App Start & Session Hydration, Auth: Login Flow, Auth: Logout Flow, Auth: Token Refresh (Background), Flow Inventory, Navigation Graph, Navigation Graph (+11 more)

### Community 25 - "Cache Layer — Frontend Specification"
Cohesion: 0.10
Nodes (19): Accessibility, Behaviour, Cache Layer — Frontend Specification, CacheProvider, Components, Requirement: CacheInvalidation, Requirement: CacheSize, Requirement: CacheWrites (+11 more)

### Community 26 - "Issues Store — Frontend Specification"
Cohesion: 0.10
Nodes (19): Accessibility, Behaviour, Components, Issues Store — Frontend Specification, IssuesStore, Requirement: IssuesCollection, Requirement: IssuesFilters, Requirement: IssuesPagination (+11 more)

### Community 27 - "Selectors — Frontend Specification"
Cohesion: 0.10
Nodes (19): Accessibility, Behaviour, Components, Requirement: SelectorComposition, Requirement: SelectorDefinition, Requirement: SelectorTypes, Routing, Scenario: selectActiveCycle returns current cycle (+11 more)

### Community 28 - "Flow Inventory"
Cohesion: 0.10
Nodes (19): Actors, Flow Inventory, Navigation Graph, Navigation Graph, Navigation Graph, Navigation Graph, Screen List, Screen List (+11 more)

### Community 29 - "Cache Layer — Frontend Specification"
Cohesion: 0.10
Nodes (19): Accessibility, Behaviour, Cache Layer — Frontend Specification, CacheProvider, Components, Requirement: CacheInvalidation, Requirement: CacheSize, Requirement: CacheWrites (+11 more)

### Community 30 - "Requirement: IssuesStore"
Cohesion: 0.25
Nodes (8): Requirement: IssuesStore, Scenario: A new issue is created, Scenario: An existing issue is updated, Scenario: An issue is deleted, Scenario: Deselect issue, Scenario: Load issues, Scenario: Select an issue, Scenario: Store initialises with empty state

### Community 31 - "Selectors — Frontend Specification"
Cohesion: 0.10
Nodes (19): Accessibility, Behaviour, Components, Requirement: SelectorComposition, Requirement: SelectorDefinition, Requirement: SelectorTypes, Routing, Scenario: selectActiveCycle returns current cycle (+11 more)

### Community 32 - "Containerized SPA Serving — Frontend Specification"
Cohesion: 0.11
Nodes (18): Accessibility, Behaviour, Components, Containerized SPA Serving — Frontend Specification, Dockerfile, Nginx Server Configuration, Requirement: HiddenFilesDenied, Requirement: NonRootUser (+10 more)

### Community 33 - "Containerized SPA Serving — Frontend Specification"
Cohesion: 0.11
Nodes (18): Accessibility, Behaviour, Components, Containerized SPA Serving — Frontend Specification, Dockerfile, Nginx Server Configuration, Requirement: HiddenFilesDenied, Requirement: NonRootUser (+10 more)

### Community 34 - "Store Architecture — Frontend Specification"
Cohesion: 0.12
Nodes (16): Accessibility, Behaviour, Components, Requirement: DomainStores, Requirement: StoreInterface, Routing, Scenario: Define store with initial state, Scenario: Mutate state via action (+8 more)

### Community 35 - "Wireframe: {ScreenName}"
Cohesion: 0.12
Nodes (16): Accessibility — Screen Composition, Accessibility — Shared Conventions, Components & Affordances, Content & Microcopy Intent, Data Display, Empty, Error, Layout Regions (+8 more)

### Community 36 - "Store Architecture — Frontend Specification"
Cohesion: 0.12
Nodes (16): Accessibility, Behaviour, Components, Requirement: DomainStores, Requirement: StoreInterface, Routing, Scenario: Define store with initial state, Scenario: Mutate state via action (+8 more)

### Community 37 - "MSW Handlers — Frontend Design"
Cohesion: 0.12
Nodes (15): Accessibility, Architecture Decisions, Asset Map, `browser.ts`, Data Fetching, Decision 1: Single handlers array in `handlers.ts`, Decision 2: Separate runtime files per MSW target, Decision 3: API base path as a shared constant (+7 more)

### Community 38 - "Browser Mode: Launch Dev App with Mocked API"
Cohesion: 0.12
Nodes (15): Actors, Browser Mode: Launch Dev App with Mocked API, Flow Inventory, Mock Management: Add a New Handler, Mock Management: Override a Handler in a Test, Navigation Graph, Navigation Graph, Navigation Graph (+7 more)

### Community 39 - "Tasks — State Module: Cache & Selectors (Frontend)"
Cohesion: 0.12
Nodes (15): Components — AuthGuard, Components — StoreProvider, Integration, Review, Routing, Scaffold, State & Data — Auth Store, State & Data — Cache Layer (+7 more)

### Community 40 - "API Client: Error Handling Flow"
Cohesion: 0.12
Nodes (15): Actors, API Client: Error Handling Flow, API Client: Rate Limit Tracking Flow, API Client: Token Refresh Flow, Flow Inventory, Navigation Graph, Navigation Graph, Navigation Graph (+7 more)

### Community 41 - "Component Catalog"
Cohesion: 0.12
Nodes (15): Button, Card, Checkbox, Color Semantics, Component Catalog, Design System — Linear App Clone (Frontend), EmptyState, Input (+7 more)

### Community 42 - "Behaviour"
Cohesion: 0.05
Nodes (40): Accessibility, Behaviour, Components, Header, Layout Module — Frontend Specification, PageLayout, Requirement: PageLayoutStructure, Requirement: ResponsiveBreakpoints (+32 more)

### Community 43 - "Component Catalog"
Cohesion: 0.12
Nodes (15): Backdrop, Component Catalog, Design System — Linear App Clone (Frontend), HamburgerButton, Header, Layout Tokens, NavLink, NotificationBadge (+7 more)

### Community 44 - "Available Features"
Cohesion: 0.12
Nodes (15): Authentication & Session, Available Features, Build, Client API, Docker, Form Validation, Infrastructure, Linear App Clone — Frontend (+7 more)

### Community 45 - "stores/selectors/index.ts"
Cohesion: 0.16
Nodes (14): ActiveFilters, Cycle, ProjectProgress, selectActiveCycle, selectFilteredIssues, selectIssuesByStatus, selectProjectProgress, selectUnreadCount (+6 more)

### Community 46 - "Routing — Frontend Design"
Cohesion: 0.13
Nodes (14): Accessibility, Active Route Hook (`useActiveRoute`), Architecture Decisions, Asset Map, Component Tree, Data Fetching, Page Components, Post-Login Redirect State (+6 more)

### Community 47 - "WebSocket Store — Frontend Specification"
Cohesion: 0.13
Nodes (14): Accessibility, Behaviour, Components, Requirement: ConnectionState, Requirement: Notifications, Routing, Scenario: Mark notification as read, Scenario: Receive notification (+6 more)

### Community 48 - "WebSocket Store — Frontend Specification"
Cohesion: 0.13
Nodes (14): Accessibility, Behaviour, Components, Requirement: ConnectionState, Requirement: Notifications, Routing, Scenario: Mark notification as read, Scenario: Receive notification (+6 more)

### Community 49 - "ADR-0008: API Error Taxonomy via Class Hierarchy"
Cohesion: 0.14
Nodes (13): ADR-0008: API Error Taxonomy via Class Hierarchy, Callback object (ErrorHandler), Class hierarchy (ApiError base + typed subclasses), Confirmation, Consequences, Considered Options, Context and Problem Statement, Decision Drivers (+5 more)

### Community 50 - "Frontend Architecture — Linear App Clone"
Cohesion: 0.12
Nodes (16): Component Design, Current Decisions, Data Flow, Files Affected by WebSocket Alignment, Frontend, Frontend Architecture — Linear App Clone, Frontend — Component Architecture, Overview (+8 more)

### Community 51 - "Component Catalog"
Cohesion: 0.14
Nodes (13): Color Semantics, Component Catalog, ConnectionIndicator, Design System — Linear App Clone, DropdownMenu, EmptyState, ErrorBanner, Layout Tokens (+5 more)

### Community 52 - "Auth Store — Frontend Specification"
Cohesion: 0.14
Nodes (13): Accessibility, Auth Store — Frontend Specification, AuthStore, Behaviour, Components, Requirement: AuthState, Routing, Scenario: Authenticated state after login (+5 more)

### Community 53 - "Architecture"
Cohesion: 0.14
Nodes (13): Architecture, Backend, Backend — {hexagonal / clean} architecture, Component Design, Current Decisions, Data Flow, Frontend, Frontend — {Next.js App Router} (+5 more)

### Community 54 - "Auth Store — Frontend Specification"
Cohesion: 0.14
Nodes (13): Accessibility, Auth Store — Frontend Specification, AuthStore, Behaviour, Components, Requirement: AuthState, Routing, Scenario: Authenticated state after login (+5 more)

### Community 55 - "Header/index.ts"
Cohesion: 0.23
Nodes (9): Header(), HeaderProps, NotificationBell(), NotificationBellProps, SearchTrigger(), SearchTriggerProps, themeConfig, themeCycle (+1 more)

### Community 56 - "ADR-0004: Feature-Sliced Design Architecture"
Cohesion: 0.15
Nodes (12): ADR-0004: Feature-Sliced Design Architecture, Confirmation, Consequences, Considered Options, Context and Problem Statement, Decision Drivers, Decision Outcome, Feature-Sliced Design (FSD) (+4 more)

### Community 57 - "ADR-0005: React Hook Form + Zod for Form Management"
Cohesion: 0.15
Nodes (12): ADR-0005: React Hook Form + Zod for Form Management, Confirmation, Consequences, Considered Options, Context and Problem Statement, Decision Drivers, Decision Outcome, Formik (+4 more)

### Community 58 - "Component Catalog"
Cohesion: 0.15
Nodes (12): AuthGuard, Button, Color Semantics, Component Catalog, Design System — Linear App Clone, ErrorBanner, Layout Tokens, LoginForm (+4 more)

### Community 59 - "Component Catalog"
Cohesion: 0.15
Nodes (12): AuthGuard, Color Semantics, Component Catalog, ConnectionIndicator, Design System — Linear App Clone, EmptyState, ErrorBanner, HeaderBar (+4 more)

### Community 60 - "Auth — Frontend Design"
Cohesion: 0.15
Nodes (12): Accessibility, Architecture Decisions, Asset Map, Auth — Frontend Design, Component Tree, Data Fetching, MSW Handlers (create src/mocks/handlers.ts), Routing (+4 more)

### Community 61 - "session/model/store.ts"
Cohesion: 0.21
Nodes (10): AuthState, initialAuthState, useAuthStore, AuthError, LoginFormData, LoginResponse, LogoutResponse, RefreshResponse (+2 more)

### Community 62 - "ADR-0007: Store Isolation — Separate Zustand Stores per Domain"
Cohesion: 0.17
Nodes (11): ADR-0007: Store Isolation — Separate Zustand Stores per Domain, Confirmation, Consequences, Considered Options, Context and Problem Statement, Decision Drivers, Decision Outcome, More Information (+3 more)

### Community 63 - "UI Generic Components — Frontend Design"
Cohesion: 0.17
Nodes (11): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Routing, State Management, Store Interface — Modal (+3 more)

### Community 64 - "{Domain} — Frontend Specification"
Cohesion: 0.17
Nodes (11): Accessibility, Behaviour, {ComponentName}, Components, {Domain} — Frontend Specification, Requirement: {RequirementName}, Routing, Scenario: {AnotherScenarioName} (+3 more)

### Community 65 - "Atomic Parts"
Cohesion: 0.18
Nodes (11): 4.1 Store Architecture, 4.2 State Shapes, 4.3 Component Dependencies, 4.4 State Mutations, 4.5 Selectors, 4.6 State Persistence, 4.7 Cache Management, 4. State Module (+3 more)

### Community 66 - "Auth Module — Frontend Design"
Cohesion: 0.18
Nodes (10): Accessibility, Architecture Decisions, Asset Map, Auth Module — Frontend Design, AuthStore Shape, Component Tree, Data Fetching, Routing (+2 more)

### Community 67 - "Component Catalog"
Cohesion: 0.18
Nodes (10): Breadcrumb (Route Indicator), Color Semantics, Component Catalog, Design System — Linear App Clone (Frontend), Layout Tokens, NavLink (Sidebar), NotFoundPage, RouteLink (Inline) (+2 more)

### Community 68 - "Tasks — Remove localStorage Token Storage, Use HttpOnly Cookie (Frontend)"
Cohesion: 0.18
Nodes (10): hydrate method, login method, logout method, MSW Handlers, refreshAccessToken method, Session Store, Tasks — Remove localStorage Token Storage, Use HttpOnly Cookie (Frontend), Testing (+2 more)

### Community 69 - "Container Build & Run: DockerBuildFlow"
Cohesion: 0.18
Nodes (10): Actors, Container Build & Run: DockerBuildFlow, Deployment: DockerDeployFlow, Flow Inventory, Navigation Graph, Navigation Graph, Screen List, State Transitions (+2 more)

### Community 70 - "Tasks — API Client & Error Handling (Frontend)"
Cohesion: 0.18
Nodes (10): ApiClient, Error Taxonomy, ErrorHandler, Interceptors, RateLimitStore, Review, Scaffold, Tasks — API Client & Error Handling (Frontend) (+2 more)

### Community 71 - "Components"
Cohesion: 0.18
Nodes (11): Button, Card, Checkbox, Components, EmptyState, Input, LoadingIndicator, Modal (+3 more)

### Community 72 - "Deployment"
Cohesion: 0.18
Nodes (10): Architecture, Backend, Backup & Recovery, CI/CD, Deployment, Frontend, Infrastructure, Monitoring (+2 more)

### Community 73 - "Components"
Cohesion: 0.18
Nodes (11): Button, Card, Checkbox, Components, EmptyState, Input, LoadingIndicator, Modal (+3 more)

### Community 74 - "Deployment — Linear App Clone Frontend"
Cohesion: 0.17
Nodes (11): Architecture, Build Optimization, CI/CD, Deployment — Linear App Clone Frontend, Environment Configuration, Frontend, Infrastructure, Local Development (+3 more)

### Community 75 - "Atomic Parts"
Cohesion: 0.20
Nodes (10): 5.1 HTTP Client, 5.2 Request/Response Interceptors, 5.3 Error Handling, 5.4 Rate Limiting, 5.5 Data Transformation, 5.6 Request Deduplication, 5. API Module, Atomic Parts (+2 more)

### Community 76 - "Atomic Parts"
Cohesion: 0.20
Nodes (10): 6.1 Design System Components, 6.2 Layout Components, 6.3 Data Display Components, 6.4 Form Components, 6.5 Loading States, 6.6 Empty States, 6. UI Module, Atomic Parts (+2 more)

### Community 77 - "State Module: Cache & Selectors — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Routing, State Management, State Module: Cache & Selectors — Frontend Design (+1 more)

### Community 78 - "Tasks — Add Dockerfile for Containerization (Frontend)"
Cohesion: 0.20
Nodes (9): Build Stage (Dockerfile), Dockerignore, Documentation, Nginx Configuration, Production Stage (Dockerfile), Review, Scaffold, Tasks — Add Dockerfile for Containerization (Frontend) (+1 more)

### Community 79 - "API Client & Error Handling — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, API Client & Error Handling — Frontend Design, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Routing, State Management (+1 more)

### Community 80 - "Flow Inventory"
Cohesion: 0.12
Nodes (15): Actors, Flow Inventory, Layout: Application Shell, Layout: Sidebar Navigation, Layout: Theme Switching, Navigation Graph, Navigation Graph, Navigation Graph (+7 more)

### Community 81 - "ADR-0001: Zustand Issues Store Following Entity-Store Pattern"
Cohesion: 0.04
Nodes (48): ADR-0001: Zustand Issues Store Following Entity-Store Pattern, ADR-0002: Separate API Layer Decoupled from Store, ADR-0003: Pessimistic Updates for Issue CRUD, ADR-0004: react-hook-form with Zod for Issue Form Validation, ADR Review Manifest, API calls inline in store actions, Confirmation, Confirmation (+40 more)

### Community 82 - "OpenSpec Schema Collection"
Cohesion: 0.20
Nodes (9): Adding a New Schema, Community Schemas, Example: Switching Schemas, From fullstack to backend-only, From fullstack to minimalist, OpenSpec Schema Collection, Quick Start, Schema Catalog (+1 more)

### Community 83 - "{Domain} — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, {Domain} — Frontend Design, Routing, State Management (+1 more)

### Community 84 - "devDependencies"
Cohesion: 0.07
Nodes (29): @commitlint/cli, eslint-plugin-boundaries, eslint-plugin-react-hooks, husky, devDependencies, @commitlint/cli, eslint-plugin-boundaries, eslint-plugin-react-hooks (+21 more)

### Community 85 - "Atomic Parts"
Cohesion: 0.22
Nodes (9): 10.1 Page Layout, 10.2 Sidebar, 10.3 Header, 10.4 Responsive Behavior, 10.5 Theme, 10. Layout Module, Atomic Parts, Purpose (+1 more)

### Community 86 - "Atomic Parts"
Cohesion: 0.22
Nodes (9): 1.1 Auth State, 1.2 Login Flow, 1.3 Logout Flow, 1.4 Token Refresh, 1.5 Auth Guard, 1. Auth Module, Atomic Parts, Purpose (+1 more)

### Community 87 - "Atomic Parts"
Cohesion: 0.22
Nodes (9): 8.1 Shortcut Registry, 8.2 Shortcut Execution, 8.3 Context Management, 8.4 Shortcut Help, 8.5 Shortcut Customization, 8. Keyboard Module, Atomic Parts, Purpose (+1 more)

### Community 88 - "Frontend Stack — Linear App Clone"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Frontend Stack — Linear App Clone, Generated Files, Interactive Review Log, Next Steps

### Community 89 - "Tasks — Routing Module (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — Routing Module (Frontend), Validation

### Community 90 - "Design System — Linear App Clone (Frontend)"
Cohesion: 0.22
Nodes (8): Button, Color Semantics, Component Catalog, Design System — Linear App Clone (Frontend), ErrorBanner, Layout Tokens, Toast, Typography

### Community 91 - "Behaviour"
Cohesion: 0.22
Nodes (9): Behaviour, Requirement: Card, Requirement: Checkbox, Requirement: Empty State, Scenario: Card is clickable, Scenario: Card renders content, Scenario: Checkbox shows indeterminate state, Scenario: Empty state displays message (+1 more)

### Community 92 - "Tasks — UI Generic Components (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — UI Generic Components (Frontend), Validation

### Community 93 - "Tasks — {Change Title} (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — {Change Title} (Frontend), Validation

### Community 94 - "{Domain}: {FlowName}"
Cohesion: 0.22
Nodes (8): Actors, {Domain}: {FlowName}, {Domain}: {FlowName}, Flow Inventory, Navigation Graph, Screen List, State Transitions, User Flows — {Change Title}

### Community 95 - "Behaviour"
Cohesion: 0.22
Nodes (9): Behaviour, Requirement: Card, Requirement: Checkbox, Requirement: Empty State, Scenario: Card is clickable, Scenario: Card renders content, Scenario: Checkbox shows indeterminate state, Scenario: Empty state displays message (+1 more)

### Community 96 - "frontend/README.md"
Cohesion: 0.25
Nodes (7): Data Flow Examples, Example 1: Loading Issues Page, Example 2: Creating an Issue, Example 3: Changing Issue Status, Frontend Modules - Technology Agnostic Specification, Module Interaction Map, Module Overview

### Community 97 - "Atomic Parts"
Cohesion: 0.25
Nodes (8): 2.1 Route Definitions, 2.2 Navigation, 2.3 Route Parameters, 2.4 Route Guards, 2. Routing Module, Atomic Parts, Purpose, Responsibility

### Community 98 - "Atomic Parts"
Cohesion: 0.25
Nodes (8): 7.1 Connection Management, 7.2 Event Subscription, 7.3 Event Processing, 7.4 Optimistic Updates, 7. Realtime Module, Atomic Parts, Purpose, Responsibility

### Community 99 - "Atomic Parts"
Cohesion: 0.25
Nodes (8): 9.1 Form State, 9.2 Validation, 9.3 Form Submission, 9.4 Field Components, 9. Form Module, Atomic Parts, Purpose, Responsibility

### Community 100 - "ADR Review Manifest"
Cohesion: 0.25
Nodes (7): ADR-0001: React 19 + Vite + pnpm, ADR-0002: Zustand for State Management, ADR-0003: Tailwind CSS + shadcn/ui, ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 101 - "Tasks — Auth Module (Frontend)"
Cohesion: 0.25
Nodes (7): Components, Review, Routing, Scaffold, State & Data, Tasks — Auth Module (Frontend), Validation

### Community 102 - "Containerization — Frontend Design"
Cohesion: 0.25
Nodes (7): Architecture Decisions, Build & Run Flow, Containerization — Frontend Design, Dockerfile Structure, File Inventory, Nginx Configuration Design, Security Considerations

### Community 103 - "Tech Selection — Add Dockerfile for Containerization"
Cohesion: 0.25
Nodes (7): Added, Container Image Details, Decision Summary, Deployment Architecture, Stack Changes, Tech Selection — Add Dockerfile for Containerization, Unchanged

### Community 104 - "Layout Module — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Layout Module — Frontend Design, Routing, State Management (+1 more)

### Community 105 - "Design System — {Project Name}"
Cohesion: 0.25
Nodes (7): Color Semantics, Component Catalog, {ComponentName}, {ComponentName}, Design System — {Project Name}, Layout Tokens, Typography

### Community 106 - "Mockups — {Domain}"
Cohesion: 0.25
Nodes (7): Conventions, File structure, Mockups — {Domain}, Per-screen behavior, Responsive testing, Screen template (per file), State simulation

### Community 107 - "Review — Auth Module"
Cohesion: 0.29
Nodes (6): Artifact Coherence, Checklist, Edge Cases, Leakage Check, Review — Auth Module, Spec Compliance

### Community 108 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 109 - "Review — Routing Module"
Cohesion: 0.29
Nodes (6): Checklist, Edge Cases, Implementation Readiness, Leakage Check, Review — Routing Module, Spec Compliance

### Community 110 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 111 - "Tasks — Centralize MSW Handlers (Frontend)"
Cohesion: 0.29
Nodes (6): Integration, Review, Scaffold, State & Data, Tasks — Centralize MSW Handlers (Frontend), Validation

### Community 112 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 113 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 114 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 115 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 116 - "Tasks — Layout Module Page Structure & Theme (Frontend)"
Cohesion: 0.20
Nodes (9): Accessibility, Components, Responsive Layout, Review, Routing, State & Data, Tasks — Layout Module Page Structure & Theme (Frontend), Testing (+1 more)

### Community 117 - "Tech Selection — {Project Name} (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — {Project Name} (Frontend)

### Community 118 - "Stack"
Cohesion: 0.29
Nodes (6): Backend, Dev & Build, Frontend, Shared, Stack, Testing

### Community 119 - "Screen: {ScreenName}"
Cohesion: 0.29
Nodes (6): Elements, Layout, Navigation Flow, Screen: {ScreenName}, States, Wireframes — {Domain}

### Community 120 - "issue/api/index.ts"
Cohesion: 0.14
Nodes (4): FetchIssuesParams, CreateIssueData, PaginationCursor, UpdateIssueData

### Community 121 - "handlers.ts"
Cohesion: 0.38
Nodes (4): worker, handlers, mockIssues, server

### Community 122 - "3. Work Module"
Cohesion: 0.33
Nodes (6): 3.1 Issue Management, 3.2 Issue Status Changes, 3. Work Module, Atomic Parts, Purpose, Responsibility

### Community 123 - "Auth Module — Login & Token Management"
Cohesion: 0.33
Nodes (5): Auth Module — Login & Token Management, Impact, Motivation, Problem Statement, Scope

### Community 124 - "Routing Module — Navigation & Route Matching"
Cohesion: 0.33
Nodes (5): Impact, Motivation, Problem Statement, Routing Module — Navigation & Route Matching, Scope

### Community 125 - "Centralize MSW Handlers"
Cohesion: 0.33
Nodes (5): Centralize MSW Handlers, Impact, Motivation, Problem Statement, Scope

### Community 126 - "Review — Centralize MSW Handlers"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Centralize MSW Handlers, Spec Compliance

### Community 127 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.33
Nodes (5): ADR References, Decision Summary, Generated Files, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 128 - "Remove localStorage Token Storage, Use HttpOnly Cookie via credentials: 'include'"
Cohesion: 0.33
Nodes (5): Impact, Motivation, Problem Statement, Remove localStorage Token Storage, Use HttpOnly Cookie via credentials: 'include', Scope

### Community 129 - "Review — Remove localStorage Token Storage, Use HttpOnly Cookie via credentials: 'include'"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Remove localStorage Token Storage, Use HttpOnly Cookie via credentials: 'include', Spec Compliance

### Community 130 - "State Module — Cache & Selectors"
Cohesion: 0.33
Nodes (5): Impact, Motivation, Problem Statement, Scope, State Module — Cache & Selectors

### Community 131 - "Review — State Module: Cache & Selectors"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — State Module: Cache & Selectors, Spec Compliance

### Community 132 - "Design System — Add Dockerfile for Containerization"
Cohesion: 0.33
Nodes (5): Accessibility, Component Inventory, Design System — Add Dockerfile for Containerization, Layout Tokens, Rationale

### Community 133 - "Add Dockerfile for Containerization"
Cohesion: 0.33
Nodes (5): Add Dockerfile for Containerization, Impact, Motivation, Problem Statement, Scope

### Community 134 - "Review — Add Dockerfile for Containerization"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Add Dockerfile for Containerization, Spec Compliance

### Community 135 - "API Client & Error Handling Module"
Cohesion: 0.33
Nodes (5): API Client & Error Handling Module, Impact, Motivation, Problem Statement, Scope

### Community 136 - "Review — API Client & Error Handling"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — API Client & Error Handling, Spec Compliance

### Community 137 - "UI Module — Generic Components"
Cohesion: 0.33
Nodes (5): Impact, Motivation, Problem Statement, Scope, UI Module — Generic Components

### Community 138 - "Review — UI Generic Components"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — UI Generic Components, Spec Compliance

### Community 139 - "UI Module — Frontend Specification"
Cohesion: 0.33
Nodes (5): Accessibility, Routing, UI Module — Frontend Specification, User Flow, Validation Rules

### Community 140 - "Review — Layout Module Page Structure & Theme"
Cohesion: 0.25
Nodes (7): Artifact Coherence, Checklist, Edge Cases, Gaps, Leakage Check, Review — Layout Module Page Structure & Theme, Spec Compliance

### Community 141 - "{Change Title}"
Cohesion: 0.33
Nodes (5): {Change Title}, Impact, Motivation, Problem Statement, Scope

### Community 142 - "Review — {Change Title}"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — {Change Title}, Spec Compliance

### Community 143 - "UI Module — Frontend Specification"
Cohesion: 0.33
Nodes (5): Accessibility, Routing, UI Module — Frontend Specification, User Flow, Validation Rules

### Community 144 - "main.tsx"
Cohesion: 0.47
Nodes (3): App(), StoreProvider(), StoreProviderProps

### Community 145 - "useLoginForm.ts"
Cohesion: 0.47
Nodes (4): useAuth(), LoginFormData, loginSchema, useLoginForm()

### Community 146 - "cacheStore.ts"
Cohesion: 0.33
Nodes (4): CacheEntry, CacheState, DEFAULT_TTLS, useCacheStore

### Community 147 - "websocketStore.ts"
Cohesion: 0.33
Nodes (5): ConnectionStatus, initialWebSocketState, Notification, useWebSocketStore, WebSocketState

### Community 148 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 149 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 150 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 151 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 152 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 153 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 154 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 155 - "Requirement: Button Variants"
Cohesion: 0.40
Nodes (5): Requirement: Button Variants, Scenario: Danger variant conveys destructive action, Scenario: Disabled button does not respond to clicks, Scenario: Primary button triggers action, Scenario: Secondary variant renders with subdued styling

### Community 156 - "Requirement: Modal with Stack Management"
Cohesion: 0.40
Nodes (5): Requirement: Modal with Stack Management, Scenario: Modal closes on backdrop click, Scenario: Modal closes on Esc, Scenario: Modal opens on trigger, Scenario: Nested modals stack correctly

### Community 157 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 158 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 159 - "Requirement: Button Variants"
Cohesion: 0.40
Nodes (5): Requirement: Button Variants, Scenario: Danger variant conveys destructive action, Scenario: Disabled button does not respond to clicks, Scenario: Primary button triggers action, Scenario: Secondary variant renders with subdued styling

### Community 160 - "Requirement: Modal with Stack Management"
Cohesion: 0.40
Nodes (5): Requirement: Modal with Stack Management, Scenario: Modal closes on backdrop click, Scenario: Modal closes on Esc, Scenario: Modal opens on trigger, Scenario: Nested modals stack correctly

### Community 161 - "toastStore.ts"
Cohesion: 0.40
Nodes (4): Toast, ToastState, ToastVariant, useToastStore

### Community 162 - "uiStore.ts"
Cohesion: 0.40
Nodes (4): KeyboardContext, Theme, UIState, useUIStore

### Community 163 - "ErrorBanner.tsx"
Cohesion: 0.40
Nodes (3): ErrorBannerProps, ErrorBannerType, typeStyles

### Community 164 - "Requirement: Input Field"
Cohesion: 0.50
Nodes (4): Requirement: Input Field, Scenario: Disabled input does not accept input, Scenario: Input shows error state, Scenario: User types into input

### Community 165 - "Requirement: Toast Notifications"
Cohesion: 0.50
Nodes (4): Requirement: Toast Notifications, Scenario: Toast appears on trigger, Scenario: Toast variants show correct styling, Scenario: User dismisses toast manually

### Community 166 - "Technology Templates — Frontend Schema"
Cohesion: 0.50
Nodes (3): Customizing Templates, Fallback Behavior, Technology Templates — Frontend Schema

### Community 167 - "Requirement: Input Field"
Cohesion: 0.50
Nodes (4): Requirement: Input Field, Scenario: Disabled input does not accept input, Scenario: Input shows error state, Scenario: User types into input

### Community 168 - "Requirement: Toast Notifications"
Cohesion: 0.50
Nodes (4): Requirement: Toast Notifications, Scenario: Toast appears on trigger, Scenario: Toast variants show correct styling, Scenario: User dismisses toast manually

### Community 170 - "modalStore.ts"
Cohesion: 0.50
Nodes (3): ModalStackItem, ModalState, useModalStore

### Community 171 - "stores/rate-limit.ts"
Cohesion: 0.50
Nodes (3): EndpointRateLimit, RateLimitState, useRateLimitStore

### Community 173 - "AGENTS.md"
Cohesion: 0.10
Nodes (20): 1. CodeGraph + Graphify (Code Intelligence), 2. Memory (Persistent Knowledge), 3. RTK - Optimized Commands (Token Savings), Analysis (70-90% savings), Analysis (70-90% savings), Build & Lint (80-90% savings) — shows errors only, Build & Lint (80-90% savings) — shows errors only, Files & Search (60-75% savings) (+12 more)

### Community 176 - "Requirement: Loading Indicator"
Cohesion: 0.67
Nodes (3): Requirement: Loading Indicator, Scenario: Full-page loader covers content, Scenario: Spinner shows during loading

### Community 177 - "Requirement: Select Dropdown"
Cohesion: 0.67
Nodes (3): Requirement: Select Dropdown, Scenario: Select shows placeholder when no option chosen, Scenario: User selects an option

### Community 178 - "Requirement: Textarea"
Cohesion: 0.67
Nodes (3): Requirement: Textarea, Scenario: Textarea shows character count, Scenario: User types into textarea

### Community 179 - "Requirement: Loading Indicator"
Cohesion: 0.67
Nodes (3): Requirement: Loading Indicator, Scenario: Full-page loader covers content, Scenario: Spinner shows during loading

### Community 180 - "Requirement: Select Dropdown"
Cohesion: 0.67
Nodes (3): Requirement: Select Dropdown, Scenario: Select shows placeholder when no option chosen, Scenario: User selects an option

### Community 181 - "Requirement: Textarea"
Cohesion: 0.67
Nodes (3): Requirement: Textarea, Scenario: Textarea shows character count, Scenario: User types into textarea

### Community 187 - "Issue Labels — Frontend Specification"
Cohesion: 0.08
Nodes (24): Accessibility, Behaviour, Components, Issue Labels — Frontend Specification, LabelBadge, LabelList, LabelPicker, Requirement: AttachLabelToIssue (+16 more)

### Community 192 - "Issue Labels — Frontend Specification"
Cohesion: 0.08
Nodes (24): Accessibility, Behaviour, Components, Issue Labels — Frontend Specification, LabelBadge, LabelList, LabelPicker, Requirement: AttachLabelToIssue (+16 more)

### Community 194 - "form/ui/index.ts"
Cohesion: 0.08
Nodes (27): useDebounce(), useFormFormik(), UseFormFormikOptions, UseFormFormikReturn, AsyncFormData, asyncValidationSchema, AsyncValidationTestForm(), DoubleSubmitTestForm() (+19 more)

### Community 195 - "Behaviour"
Cohesion: 0.11
Nodes (18): Behaviour, Field Wrapper Components — Frontend Specification, Requirement: CheckboxField Wrapper, Requirement: Field Wrapper Accessibility, Requirement: SelectField Wrapper, Requirement: TextareaField Wrapper, Requirement: TextField Wrapper, Scenario: CheckboxField Shows Error State (+10 more)

### Community 198 - "Flow Inventory"
Cohesion: 0.10
Nodes (19): Actors, Flow Inventory, Issues: Attach Label, Issues: Detach Label, Issues: Manage Labels in Issue Form, Issues: View Issue Labels, Navigation Graph, Navigation Graph (+11 more)

### Community 199 - "Behaviour"
Cohesion: 0.11
Nodes (18): Behaviour, Field Wrapper Components — Frontend Specification, Requirement: CheckboxField Wrapper, Requirement: Field Wrapper Accessibility, Requirement: SelectField Wrapper, Requirement: TextareaField Wrapper, Requirement: TextField Wrapper, Scenario: CheckboxField Shows Error State (+10 more)

### Community 201 - "Component Catalog"
Cohesion: 0.13
Nodes (14): Button (existing — extended for labels), Color Semantics, Component Catalog, Design System — Linear App Clone, ErrorBanner (existing), LabelBadge, LabelList, LabelPicker (+6 more)

### Community 203 - "Flow Inventory"
Cohesion: 0.12
Nodes (15): Actors, Flow Inventory, Form: Async Validation Flow, Form: Double Submission Prevention, Form: Standard Form Submission, Navigation Graph, Navigation Graph, Navigation Graph (+7 more)

### Community 205 - "Label Management — Frontend Design"
Cohesion: 0.15
Nodes (12): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, File Map, Label Data Model, Label Management — Frontend Design (+4 more)

### Community 206 - "form/model/index.ts"
Cohesion: 0.22
Nodes (8): CreateIssueFormData, createIssueSchema, EditIssueFormData, editIssueSchema, ProfileFormData, profileSchema, ProjectSettingsFormData, projectSettingsSchema

### Community 207 - "useKeyboardStore.ts"
Cohesion: 0.08
Nodes (22): getCurrentSequence(), handleKeyInSequence(), isInSequence(), resetSequence(), SequenceState, state, Shortcut, ShortcutContext (+14 more)

### Community 209 - "Comment Deletion — Frontend Specification"
Cohesion: 0.10
Nodes (19): Accessibility, Behaviour, Comment Deletion — Frontend Specification, CommentCard (modified), CommentList (modified), Components, DeleteConfirmOverlay (new — inline within CommentCard), Requirement: DeleteButton (+11 more)

### Community 210 - "Keyboard Module — Frontend Design"
Cohesion: 0.06
Nodes (34): Accessibility, API Endpoints (Keyboard Module), Architecture Decisions, ARIA, Asset Map, Bundle Size Considerations, Component Tree, Data Fetching (+26 more)

### Community 211 - "Edit Comment — Frontend Specification"
Cohesion: 0.11
Nodes (18): Accessibility, Behaviour, CommentCard, CommentList, Components, Edit Comment — Frontend Specification, Requirement: EditButtonVisibility, Requirement: EditCommentInline (+10 more)

### Community 237 - "Layout Module — Page Structure & Theme"
Cohesion: 0.33
Nodes (5): Impact, Layout Module — Page Structure & Theme, Motivation, Problem Statement, Scope

### Community 238 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 239 - "Behaviour"
Cohesion: 0.05
Nodes (42): Accessibility, Behaviour, Components, IssueCard, IssueDetail, IssueFilters, IssueForm, IssueList (+34 more)

### Community 240 - "Flow Inventory"
Cohesion: 0.10
Nodes (19): Actors, Flow Inventory, Navigation Graph, Navigation Graph, Navigation Graph, Navigation Graph, Screen List, Screen List (+11 more)

### Community 241 - "Component Catalog"
Cohesion: 0.11
Nodes (17): Avatar, Badge, Button, Color Semantics, Component Catalog, ConfirmDeleteDialog, Design System — Linear App Clone (Work Module), Input (+9 more)

### Community 242 - "Tasks — Work Module: Issues Store & UI (Frontend)"
Cohesion: 0.17
Nodes (11): Components, E2E tests, Integration tests, Pages, Review, Routing, Scaffold, State & Data (+3 more)

### Community 243 - "Work Module: Issues Store & UI — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Routing, State Management, Validation Strategy (+1 more)

### Community 244 - "Review — Work Module: Issues Store & UI"
Cohesion: 0.29
Nodes (6): Artifact Completeness, Checklist, Edge Cases, Leakage Check, Review — Work Module: Issues Store & UI, Spec Compliance

### Community 245 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 246 - "Work Module: Issues Store & UI Components"
Cohesion: 0.33
Nodes (5): Impact, Motivation, Problem Statement, Scope, Work Module: Issues Store & UI Components

### Community 248 - "Authentication — Frontend Specification"
Cohesion: 0.07
Nodes (28): Accessibility, Authentication — Frontend Specification, Behaviour, Components, RegisterForm, RegisterPage, Requirement: Client-Side Validation, Requirement: Loading State (+20 more)

### Community 249 - "Dedicated Status Transition Endpoint for Issue Workflow"
Cohesion: 0.15
Nodes (12): Confirmation, Consequences, Considered Options, Context and Problem Statement, Decision Drivers, Decision Outcome, Dedicated endpoint, Dedicated Status Transition Endpoint for Issue Workflow (+4 more)

### Community 250 - "MockWSEndpoint"
Cohesion: 0.07
Nodes (6): MockWSEndpoint, MockWSServer, MockWSEndpoint, MockWSServer, MockWSEndpoint, MockWSServer

### Community 251 - "IssueDetail.tsx"
Cohesion: 0.18
Nodes (9): CommentList(), IssueDetailProps, priorityLabels, IssueStatusBadge(), IssueStatusBadgeProps, STATUS_OPTIONS, statusColorMap, SkeletonLoader() (+1 more)

### Community 252 - "Issues — Frontend Specification"
Cohesion: 0.14
Nodes (13): Accessibility, Behaviour, Components, Issues — Frontend Specification, Requirement: LoadIssuesUsesApiClient, Requirement: LoadNextPageUsesApiClient, Routing, Scenario: Cache-before-fetch is preserved (+5 more)

### Community 254 - "Issue Status — Frontend Specification"
Cohesion: 0.11
Nodes (18): Accessibility, Behaviour, Components, Issue Status — Frontend Specification, IssueStatusBadge, Purpose, Requirement: ChangeStatusFromDropdown, Requirement: StatusDropdownStates (+10 more)

### Community 255 - "Issue Status — Frontend Specification"
Cohesion: 0.11
Nodes (17): Accessibility, Behaviour, Components, Issue Status — Frontend Specification, IssueStatusBadge, Requirement: ChangeStatusFromDropdown, Requirement: StatusDropdownStates, Routing (+9 more)

### Community 256 - "Issue Assign/Unassign — Frontend Design"
Cohesion: 0.07
Nodes (27): 1. Dedicated `assignIssue` Store Action (over generic `updateIssue`), 2. Optimistic Update with Rollback (over pessimistic), 3. Cache Invalidation via `useCacheStore.invalidateByPrefix('issues:list')`, 4. Business Rule Error Handling (422) via Toast, 5. No New UI Components, Accessibility, API Layer — `src/entities/issue/api/index.ts`, Architecture Decisions (+19 more)

### Community 257 - "Edit Comment — Frontend Specification"
Cohesion: 0.11
Nodes (18): Accessibility, Behaviour, CommentCard, CommentList, Components, Edit Comment — Frontend Specification, Requirement: EditButtonVisibility, Requirement: EditCommentInline (+10 more)

### Community 258 - "Issue Status Transition — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Issue Status Transition — Frontend Design, Routing, State Management (+1 more)

### Community 259 - "Component Catalog"
Cohesion: 0.20
Nodes (9): Color Semantics, Component Catalog, Design System — Linear App Clone, IssueStatusBadge, Layout Tokens, Select (existing — integration note), Spinner (existing — integration note), Toast (existing — integration note) (+1 more)

### Community 260 - "Tasks — Implement Issue Status Transitions (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — Implement Issue Status Transitions (Frontend), Validation

### Community 261 - "Issue Detail: Change Issue Status"
Cohesion: 0.25
Nodes (7): Actors, Flow Inventory, Issue Detail: Change Issue Status, Navigation Graph, Screen List, State Transitions, User Flows — Implement Issue Status Transitions

### Community 262 - "Issue Assign — Frontend Specification"
Cohesion: 0.10
Nodes (20): Accessibility, Behaviour, Components, Issue Assign — Frontend Specification, IssueDetailPage (modified), Purpose, Requirement: AssignIssueApiFunction, Requirement: AssignIssueStoreAction (+12 more)

### Community 263 - "Issue Assign — Frontend Specification"
Cohesion: 0.10
Nodes (19): Accessibility, Behaviour, Components, Issue Assign — Frontend Specification, IssueDetailPage (modified), Requirement: AssignIssueApiFunction, Requirement: AssignIssueStoreAction, Requirement: IssueDetailPageAssignment (+11 more)

### Community 264 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 265 - "Implement Issue Status Transitions"
Cohesion: 0.33
Nodes (5): Impact, Implement Issue Status Transitions, Motivation, Problem Statement, Scope

### Community 266 - "Review — Implement Issue Status Transitions"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Implement Issue Status Transitions, Spec Compliance

### Community 267 - "Dedicated Assignee Transition Endpoint for Issue Assignment"
Cohesion: 0.15
Nodes (12): Confirmation, Consequences, Considered Options, Context and Problem Statement, Decision Drivers, Decision Outcome, Dedicated Assignee Transition Endpoint for Issue Assignment, Dedicated endpoint (+4 more)

### Community 268 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 269 - "Issue Management: Change Assignee"
Cohesion: 0.17
Nodes (11): Actors, Flow Inventory, Issue Management: Change Assignee, Issue Management: Create Issue with Assignee, Navigation Graph, Navigation Graph, Screen List, Screen List (+3 more)

### Community 270 - "Design System — Linear App Clone Frontend"
Cohesion: 0.18
Nodes (10): Button, Color Semantics, Component Catalog, Design System — Linear App Clone Frontend, Layout Tokens, Modal (IssueFormModal), No New Components, Select (+2 more)

### Community 271 - "Tasks — Issue Assign/Unassign Endpoint (Frontend)"
Cohesion: 0.25
Nodes (7): API Layer, Page Integration, Review, Store, Tasks — Issue Assign/Unassign Endpoint (Frontend), Testing, Validation

### Community 272 - "Review — Issue Assign/Unassign Endpoint"
Cohesion: 0.29
Nodes (6): Architecture Consistency, Checklist, Edge Cases, Leakage Check, Review — Issue Assign/Unassign Endpoint, Spec Compliance

### Community 273 - "Implement PATCH /issues/{id}/assignee Endpoint (Assign/Unassign)"
Cohesion: 0.33
Nodes (5): Impact, Implement PATCH /issues/{id}/assignee Endpoint (Assign/Unassign), Motivation, Problem Statement, Scope

### Community 274 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.33
Nodes (5): Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 275 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 276 - "Issue Watchers — Frontend Specification"
Cohesion: 0.10
Nodes (19): Accessibility, Behaviour, Components, Issue Watchers — Frontend Specification, Requirement: ToggleWatch, Requirement: ViewWatchers, Routing, Scenario: Business rule violation on action (+11 more)

### Community 277 - "ADR-0012: Watcher Feature Slice — New FSD Entity and Feature for Issue Watchers"
Cohesion: 0.15
Nodes (12): ADR-0012: Watcher Feature Slice — New FSD Entity and Feature for Issue Watchers, Confirmation, Consequences, Considered Options, Context and Problem Statement, Decision Drivers, Decision Outcome, Extend entities/issue/ (+4 more)

### Community 278 - "WatcherList.tsx"
Cohesion: 0.24
Nodes (4): Watcher, WatcherItem(), WatcherItemProps, WatcherListProps

### Community 279 - "Label"
Cohesion: 0.13
Nodes (12): IssueLabelsState, LabelDefinitionsState, useIssueLabelsStore, useLabelDefinitionsStore, Label, LabelBadge(), LabelBadgeProps, LabelListProps (+4 more)

### Community 280 - "Issue Watchers — Frontend Specification"
Cohesion: 0.10
Nodes (19): Accessibility, Behaviour, Components, Issue Watchers — Frontend Specification, Requirement: ToggleWatch, Requirement: ViewWatchers, Routing, Scenario: Business rule violation on action (+11 more)

### Community 281 - "Issue Watchers — Frontend Design"
Cohesion: 0.17
Nodes (11): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Data Flow, Issue Watchers — Frontend Design, Routing (+3 more)

### Community 282 - "Tasks — Issue Watchers (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — Issue Watchers (Frontend), Validation

### Community 283 - "Design System — Linear App Clone"
Cohesion: 0.25
Nodes (7): Color Semantics, Component Catalog, Design System — Linear App Clone, Layout Tokens, Typography, WatchButton, WatcherList

### Community 284 - "Issue Detail: Watch/Unwatch Issue"
Cohesion: 0.25
Nodes (7): Actors, Flow Inventory, Issue Detail: Watch/Unwatch Issue, Navigation Graph, Screen List, State Transitions, User Flows — Issue Watchers

### Community 285 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 286 - "Issue Watchers"
Cohesion: 0.33
Nodes (5): Impact, Issue Watchers, Motivation, Problem Statement, Scope

### Community 287 - "WatcherSection.tsx"
Cohesion: 0.40
Nodes (3): WatchButton(), WatchButtonProps, WatcherSectionProps

### Community 288 - "Review — Issue Watchers"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Issue Watchers, Spec Compliance

### Community 289 - "watchers/model/store.ts"
Cohesion: 0.50
Nodes (3): initialWatchersState, useWatchersStore, WatchersState

### Community 290 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 291 - "Tasks — Label Management Endpoints for Issues (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — Label Management Endpoints for Issues (Frontend), Validation

### Community 292 - "Comment Deletion — Frontend Specification"
Cohesion: 0.10
Nodes (19): Accessibility, Behaviour, Comment Deletion — Frontend Specification, CommentCard (modified), CommentList (modified), Components, DeleteConfirmOverlay (new — inline within CommentCard), Requirement: DeleteButton (+11 more)

### Community 293 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 294 - "Label Management Endpoints for Issues"
Cohesion: 0.33
Nodes (5): Impact, Label Management Endpoints for Issues, Motivation, Problem Statement, Scope

### Community 295 - "Review — Label Management Endpoints for Issues"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Label Management Endpoints for Issues, Spec Compliance

### Community 296 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 297 - "Edit Comment — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Edit Comment — Frontend Design, Routing, State Management (+1 more)

### Community 298 - "Component Catalog"
Cohesion: 0.18
Nodes (10): Button, Color Semantics, CommentCard (entity-level — modified), CommentList (entity-level — modified), Component Catalog, Design System — Linear App Clone (Frontend), Inline Confirmation Pattern (CommentCard), Layout Tokens (+2 more)

### Community 299 - "Authentication — Frontend Specification"
Cohesion: 0.07
Nodes (28): Accessibility, Authentication — Frontend Specification, Behaviour, Components, RegisterForm, RegisterPage, Requirement: Client-Side Validation, Requirement: Loading State (+20 more)

### Community 301 - "Tasks — Edit Comment (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — Edit Comment (Frontend), Validation

### Community 302 - "Comments: Edit Own Comment"
Cohesion: 0.25
Nodes (7): Actors, Comments: Edit Own Comment, Flow Inventory, Navigation Graph, Screen List, State Transitions, User Flows — Edit Comment

### Community 303 - "Comment Deletion — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Comment Deletion — Frontend Design, Component Tree, Data Fetching, Routing, State Management (+1 more)

### Community 304 - "Tasks — Delete Comment (Frontend)"
Cohesion: 0.25
Nodes (7): Components, Integration, Review, Routing, State & Data, Tasks — Delete Comment (Frontend), Validation

### Community 305 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 306 - "Issue Comments: Delete Comment"
Cohesion: 0.25
Nodes (7): Actors, Flow Inventory, Issue Comments: Delete Comment, Navigation Graph, Screen List, State Transitions, User Flows — Delete Comment on Issue

### Community 307 - "Frontend Design — Register User API Integration"
Cohesion: 0.17
Nodes (11): Accessibility, Architecture Decisions, Asset Map, Auth Store Extension, Component Tree, Data Fetching, Files to Create/Modify, Frontend Design — Register User API Integration (+3 more)

### Community 308 - "Component Catalog"
Cohesion: 0.33
Nodes (5): Button (existing — referenced here for completeness), CommentCard (augmented), Component Catalog, Design System — Edit Comment, Textarea (editor)

### Community 309 - "Edit Comment"
Cohesion: 0.33
Nodes (5): Edit Comment, Impact, Motivation, Problem Statement, Scope

### Community 310 - "Review — Edit Comment"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Edit Comment, Spec Compliance

### Community 311 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 312 - "Delete Comment on Issue"
Cohesion: 0.33
Nodes (5): Delete Comment on Issue, Impact, Motivation, Problem Statement, Scope

### Community 313 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 314 - "Review — Delete Comment"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Delete Comment, Spec Compliance

### Community 315 - "Issues Module — Pagination Response Shape — Frontend Specification"
Cohesion: 0.12
Nodes (16): Accessibility, Behaviour, Components, FetchIssuesResponse, Issues Module — Pagination Response Shape — Frontend Specification, IssuesStore, Requirement: FetchIssuesResponseShape, Requirement: PaginationCursorType (+8 more)

### Community 316 - "Issues — Frontend Design"
Cohesion: 0.18
Nodes (10): Accessibility, Architecture Decisions, Change Map, Component Tree, Data Fetching, Data Flow (After Fix), Issues — Frontend Design, `src/entities/issue/api/index.ts` (+2 more)

### Community 317 - "Behaviour"
Cohesion: 0.20
Nodes (10): Behaviour, Requirement: IssueCard, Requirement: IssuesPagination, Requirement: LoadIssuesUsesApiClient, Scenario: Cache-before-fetch is preserved, Scenario: Initial load uses apiClient, Scenario: IssueCard is clickable, Scenario: IssueCard shows issue summary (+2 more)

### Community 318 - "Issues: Browse and Paginate Issues"
Cohesion: 0.25
Nodes (7): Actors, Flow Inventory, Issues: Browse and Paginate Issues, Navigation Graph, Screen List, State Transitions, User Flows — Fix: API Client Inconsistency

### Community 319 - "Issues Module — Pagination Response Shape — Frontend Specification"
Cohesion: 0.12
Nodes (16): Accessibility, Behaviour, Components, FetchIssuesResponse, Issues Module — Pagination Response Shape — Frontend Specification, IssuesStore, Requirement: FetchIssuesResponseShape, Requirement: PaginationCursorType (+8 more)

### Community 320 - "Components"
Cohesion: 0.29
Nodes (7): Components, IssueCard, IssueDetail, IssueFilters, IssueForm, IssueList, IssuesStore

### Community 321 - "Requirement: IssuesFilters"
Cohesion: 0.29
Nodes (7): Requirement: IssuesFilters, Scenario: Clear all filters, Scenario: Filter by assignee, Scenario: Filter by cycle, Scenario: Filter by labels, Scenario: Filter by project, Scenario: Filter by status

### Community 322 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 323 - "Fix: API Client Inconsistency — Store Uses Raw Fetch Instead of apiClient"
Cohesion: 0.33
Nodes (5): Fix: API Client Inconsistency — Store Uses Raw Fetch Instead of apiClient, Impact, Motivation, Problem Statement, Scope

### Community 324 - "Review — Fix: API Client Inconsistency"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Fix: API Client Inconsistency, Spec Compliance

### Community 325 - "Issues Module — Frontend Specification"
Cohesion: 0.33
Nodes (5): Accessibility, Issues Module — Frontend Specification, Routing, User Flow, Validation Rules

### Community 326 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 327 - "Tasks — Fix: API Client Inconsistency (Frontend)"
Cohesion: 0.40
Nodes (4): Review, State & Data, Tasks — Fix: API Client Inconsistency (Frontend), Validation

### Community 328 - "Requirement: IssueForm"
Cohesion: 0.40
Nodes (5): Requirement: IssueForm, Scenario: Create issue form validation, Scenario: Create issue with valid data, Scenario: Edit issue pre-fills form, Scenario: Edit issue saves changes

### Community 329 - "Issue Model — Frontend Specification"
Cohesion: 0.13
Nodes (14): Accessibility, Behaviour, Components, Issue Model — Frontend Specification, IssueType, Requirement: IssueTypeIncludesAllApiFields, Requirement: StatusFieldUsesStatusId, Routing (+6 more)

### Community 330 - "Requirement: IssueDetail"
Cohesion: 0.50
Nodes (4): Requirement: IssueDetail, Scenario: IssueDetail shows comments, Scenario: IssueDetail shows full issue information, Scenario: IssueDetail shows labels

### Community 331 - "Requirement: IssueList"
Cohesion: 0.50
Nodes (4): Requirement: IssueList, Scenario: IssueList renders fetched issues, Scenario: IssueList shows empty state, Scenario: IssueList shows loading state

### Community 333 - "Requirement: LoadNextPageUsesApiClient"
Cohesion: 0.67
Nodes (3): Requirement: LoadNextPageUsesApiClient, Scenario: Early return guards are preserved, Scenario: Pagination uses apiClient

### Community 335 - "Issues: Browse Paginated List"
Cohesion: 0.22
Nodes (8): Actors, Data Flow (Pagination), Flow Inventory, Issues: Browse Paginated List, Navigation Graph, Screen List, State Transitions, User Flows — Fix Pagination Response Shape

### Community 336 - "Issues Module — Frontend Design (Pagination Shape Fix)"
Cohesion: 0.29
Nodes (6): Affected Files, Architecture Decisions, Component Tree, Data Fetching, Issues Module — Frontend Design (Pagination Shape Fix), State Management

### Community 337 - "Fix: Pagination response shape mismatch — meta vs pagination"
Cohesion: 0.33
Nodes (5): Fix: Pagination response shape mismatch — meta vs pagination, Impact, Motivation, Problem Statement, Scope

### Community 338 - "Review — Fix Pagination Response Shape"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Fix Pagination Response Shape, Spec Compliance

### Community 339 - "Tasks — Fix Pagination Response Shape (Frontend)"
Cohesion: 0.33
Nodes (5): API Layer, Store, Tasks — Fix Pagination Response Shape (Frontend), Types, Validation

### Community 340 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 341 - "Tech Selection — Fix Pagination Response Shape"
Cohesion: 0.50
Nodes (3): Decision Summary, Generated Files, Tech Selection — Fix Pagination Response Shape

### Community 342 - "Issue API — Frontend Specification"
Cohesion: 0.17
Nodes (11): Accessibility, Behaviour, Components, deleteIssue API Function, Issue API — Frontend Specification, Requirement: DeleteIssueReturnType, Routing, Scenario: DeleteIssueReturnsVoid (+3 more)

### Community 343 - "Issue API — Frontend Specification"
Cohesion: 0.17
Nodes (11): Accessibility, Behaviour, Components, deleteIssue API Function, Issue API — Frontend Specification, Requirement: DeleteIssueReturnType, Routing, Scenario: DeleteIssueReturnsVoid (+3 more)

### Community 344 - "Issue API — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Issue API — Frontend Design, Routing, State Management (+1 more)

### Community 345 - "Tasks — Fix Delete Issue Response Type (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — Fix Delete Issue Response Type (Frontend), Validation

### Community 346 - "Issue Management: Delete Issue"
Cohesion: 0.25
Nodes (7): Actors, Flow Inventory, Issue Management: Delete Issue, Navigation Graph, Screen List, State Transitions, User Flows — Fix Delete Issue Response Type

### Community 347 - "Requirement: Validation Types"
Cohesion: 0.14
Nodes (13): Behaviour, Requirement: Validation Timing, Requirement: Validation Types, Scenario: Async Validation, Scenario: Custom Validation, Scenario: MaxLength Validation, Scenario: MinLength Validation, Scenario: On Blur Validation (+5 more)

### Community 348 - "Tech Selection — Fix Delete Issue Response Type"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Fix Delete Issue Response Type

### Community 349 - "Issues Filtering — Frontend Specification"
Cohesion: 0.12
Nodes (15): Accessibility, Behaviour, Components, IssueFilters (existing — amends), Issues Filtering — Frontend Specification, IssuesStore (existing — amends), Requirement: LabelIdsWired, Requirement: SpecOnlyParams (+7 more)

### Community 350 - "Issues Filtering — Frontend Specification"
Cohesion: 0.12
Nodes (15): Accessibility, Behaviour, Components, IssueFilters (existing — amends), Issues Filtering — Frontend Specification, IssuesStore (existing — amends), Requirement: LabelIdsWired, Requirement: SpecOnlyParams (+7 more)

### Community 351 - "Issues Filtering — Frontend Design"
Cohesion: 0.15
Nodes (12): Architecture Decisions, Changes by File, Data Flow, Issues Filtering — Frontend Design, Risk & Mitigation, `src/entities/issue/api/index.ts` — `FetchIssuesParams` + `fetchIssues`, `src/entities/issue/model/store.ts` — `loadIssues`, `loadNextPage`, `src/entities/issue/model/types.ts` — `IssueFilters` (+4 more)

### Community 352 - "Tasks — Fix Query Parameter Mismatch (Frontend)"
Cohesion: 0.25
Nodes (7): API Layer, Store, Tasks — Fix Query Parameter Mismatch (Frontend), Testing, Types, UI Components, Validation

### Community 353 - "Issues: Filter Issues List"
Cohesion: 0.25
Nodes (7): Actors, Flow Inventory, Issues: Filter Issues List, Navigation Graph, Screen List, State Transitions, User Flows — Fix: Query Parameter Mismatch

### Community 354 - "Fix: Query Parameter Mismatch — status vs statusId, extra search/priority"
Cohesion: 0.33
Nodes (5): Fix: Query Parameter Mismatch — status vs statusId, extra search/priority, Impact, Motivation, Problem Statement, Scope

### Community 355 - "Review — Fix Query Parameter Mismatch"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Fix Query Parameter Mismatch, Spec Compliance

### Community 356 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 357 - "Tech Selection — Fix Query Parameter Mismatch"
Cohesion: 0.40
Nodes (4): ADR References, Decision Summary, Existing Stack Reference, Tech Selection — Fix Query Parameter Mismatch

### Community 360 - "Issue Creation — Frontend Specification"
Cohesion: 0.12
Nodes (15): Accessibility, Behaviour, Components, Issue Creation — Frontend Specification, IssueFormModal, IssuesPage, Purpose, Requirement: CreateIssueIncludesTeamId (+7 more)

### Community 361 - "Issue Creation — Frontend Specification"
Cohesion: 0.13
Nodes (14): Accessibility, Behaviour, Components, Issue Creation — Frontend Specification, IssueFormModal, IssuesPage, Requirement: CreateIssueIncludesTeamId, Requirement: SelectedTeamIsShared (+6 more)

### Community 362 - "Issue Creation — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Issue Creation — Frontend Design, State Management, TeamStore (+1 more)

### Community 363 - "Design System — Linear App Clone"
Cohesion: 0.20
Nodes (9): Accessibility Contract, Color Semantics, Component Catalog, Design System — Linear App Clone, IssueFormModal, IssuesPage, Layout Tokens, TeamSelector (existing) (+1 more)

### Community 364 - "Issue Creation: Create Issue with Team Context"
Cohesion: 0.25
Nodes (7): Actors, Flow Inventory, Issue Creation: Create Issue with Team Context, Navigation Graph, Screen List, State Transitions, User Flows — Fix: Create Issue Missing Required teamId Field

### Community 365 - "Tasks — Fix: Create Issue Missing Required teamId Field (Frontend)"
Cohesion: 0.29
Nodes (6): Components, Integration, Review, State & Data, Tasks — Fix: Create Issue Missing Required teamId Field (Frontend), Validation

### Community 366 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 367 - "Fix: Create Issue Missing Required teamId Field"
Cohesion: 0.33
Nodes (5): Fix: Create Issue Missing Required teamId Field, Impact, Motivation, Problem Statement, Scope

### Community 368 - "Review — Fix: Create Issue Missing Required teamId Field"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Fix: Create Issue Missing Required teamId Field, Spec Compliance

### Community 370 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 371 - "Design System — Fix Delete Issue Response Type"
Cohesion: 0.33
Nodes (5): Color Semantics, Component Catalog, Design System — Fix Delete Issue Response Type, Layout Tokens, Typography

### Community 372 - "Fix: Delete Issue Response Type Mismatch"
Cohesion: 0.33
Nodes (5): Fix: Delete Issue Response Type Mismatch, Impact, Motivation, Problem Statement, Scope

### Community 373 - "Review — Fix Delete Issue Response Type"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Fix Delete Issue Response Type, Spec Compliance

### Community 374 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 375 - "Issue Model — Frontend Specification"
Cohesion: 0.13
Nodes (14): Accessibility, Behaviour, Components, Issue Model — Frontend Specification, IssueType, Requirement: IssueTypeIncludesAllApiFields, Requirement: StatusFieldUsesStatusId, Routing (+6 more)

### Community 377 - "Issue Model Alignment — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Issue Model Alignment — Frontend Design, Routing, State Management (+1 more)

### Community 378 - "Issue Model: Type Alignment"
Cohesion: 0.25
Nodes (7): Actors, Flow Inventory, Issue Model: Type Alignment, Navigation Graph, Screen List, State Transitions, User Flows — Fix: Issue Model Missing 7 Fields from API Spec

### Community 379 - "Tasks — Fix: Issue Model Missing 7 Fields from API Spec (Frontend)"
Cohesion: 0.29
Nodes (6): Component Updates, Page Updates, State & Data Updates, Tasks — Fix: Issue Model Missing 7 Fields from API Spec (Frontend), Type Definition Update, Validation

### Community 380 - "Tech Selection — Fix: Issue Model Missing Fields (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Fix: Issue Model Missing Fields (Frontend)

### Community 381 - "Design System — Fix: Issue Model Missing Fields"
Cohesion: 0.33
Nodes (5): Color Semantics, Component Catalog, Design System — Fix: Issue Model Missing Fields, Layout Tokens, Typography

### Community 382 - "Fix: Issue Model Missing 7 Fields from API Spec"
Cohesion: 0.33
Nodes (5): Fix: Issue Model Missing 7 Fields from API Spec, Impact, Motivation, Problem Statement, Scope

### Community 383 - "Review — Fix: Issue Model Missing 7 Fields from API Spec"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Fix: Issue Model Missing 7 Fields from API Spec, Spec Compliance

### Community 384 - "Requirement: Validation Types"
Cohesion: 0.14
Nodes (13): Behaviour, Requirement: Validation Timing, Requirement: Validation Types, Scenario: Async Validation, Scenario: Custom Validation, Scenario: MaxLength Validation, Scenario: MinLength Validation, Scenario: On Blur Validation (+5 more)

### Community 385 - "Behaviour"
Cohesion: 0.17
Nodes (11): Behaviour, Form State Management — Frontend Specification, Requirement: Form Validity Computation, Requirement: FormState Structure, Requirement: Touched State Tracking, Scenario: Field Marked as Touched on Blur, Scenario: Form Invalid When Any Error Exists, Scenario: Form Valid When No Errors (+3 more)

### Community 388 - "Behaviour"
Cohesion: 0.17
Nodes (11): Behaviour, Form Submission — Frontend Specification, Requirement: Double Submission Prevention, Requirement: Form Disabled During Submission, Requirement: Submission Flow, Scenario: Form Fields Disabled While Submitting, Scenario: Rapid Double Click Prevention, Scenario: Submission Failure (+3 more)

### Community 389 - "Behaviour"
Cohesion: 0.17
Nodes (11): Behaviour, Form State Management — Frontend Specification, Requirement: Form Validity Computation, Requirement: FormState Structure, Requirement: Touched State Tracking, Scenario: Field Marked as Touched on Blur, Scenario: Form Invalid When Any Error Exists, Scenario: Form Valid When No Errors (+3 more)

### Community 391 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 392 - "Behaviour"
Cohesion: 0.17
Nodes (11): Behaviour, Form Submission — Frontend Specification, Requirement: Double Submission Prevention, Requirement: Form Disabled During Submission, Requirement: Submission Flow, Scenario: Form Fields Disabled While Submitting, Scenario: Rapid Double Click Prevention, Scenario: Submission Failure (+3 more)

### Community 393 - "Form Module — Frontend Design"
Cohesion: 0.18
Nodes (10): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Flow (Form Submission), Form Module — Frontend Design, Routing, State Management (+2 more)

### Community 394 - "Component Catalog"
Cohesion: 0.18
Nodes (10): CheckboxField, Color Semantics, Component Catalog, Design System — Form Module, Layout Tokens, SelectField, SubmitButton, TextareaField (+2 more)

### Community 395 - "Tech Selection — Form Module (Validation & Submission)"
Cohesion: 0.18
Nodes (10): ADR References, Decision Summary, Field Wrapper Pattern, Form Module Technology Stack, Generated Files, Interactive Review Log, Next Steps, react-hook-form (+2 more)

### Community 396 - "EditIssuePage.tsx"
Cohesion: 0.40
Nodes (3): priorityOptions, projectOptions, statusOptions

### Community 398 - "Behaviour"
Cohesion: 0.06
Nodes (34): Accessibility, Behaviour, Components, Keyboard Module — Frontend Specification, KeyboardProvider, Requirement: Context Management, Requirement: Shortcut Customization, Requirement: Shortcut Execution (+26 more)

### Community 401 - "Tasks — Form Module: Validation & Submission (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — Form Module: Validation & Submission (Frontend), Validation

### Community 402 - "Review — Form Module: Validation & Submission"
Cohesion: 0.29
Nodes (6): Checklist, Edge Cases, Leakage Check, Notes, Review — Form Module: Validation & Submission, Spec Compliance

### Community 403 - "Form Module — Validation & Submission"
Cohesion: 0.33
Nodes (5): Form Module — Validation & Submission, Impact, Motivation, Problem Statement, Scope

### Community 404 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 405 - "Realtime Module — Event Subscription & Processing"
Cohesion: 0.07
Nodes (29): Accessibility, Behaviour, Components, EventProcessor, EventRouter, Realtime Module — Event Subscription & Processing, Requirement: Event Ordering, Requirement: Event Processing Pipeline (+21 more)

### Community 406 - "Realtime Module — Event Subscription & Processing"
Cohesion: 0.07
Nodes (29): Accessibility, Behaviour, Components, EventProcessor, EventRouter, Realtime Module — Event Subscription & Processing, Requirement: Event Ordering, Requirement: Event Processing Pipeline (+21 more)

### Community 407 - "IssueForm.tsx"
Cohesion: 0.29
Nodes (7): issueFormFieldLabels, IssueFormSchema, IssueForm(), IssueFormProps, priorityOptions, statusOptions, IssueFormModalProps

### Community 408 - "Flow Inventory"
Cohesion: 0.07
Nodes (27): Actors, Connection: WebSocket Lifecycle Management, Events: Notification Delivery, Events: Real-time Event Subscription, Flow Inventory, Navigation Graph, Navigation Graph, Navigation Graph (+19 more)

### Community 409 - "Behaviour"
Cohesion: 0.07
Nodes (26): Accessibility, Behaviour, Components, OptimisticUpdateManager, OptimisticUpdateProvider, Realtime Module — Optimistic Updates, Requirement: Duplicate Action Prevention, Requirement: Failure Revert (+18 more)

### Community 410 - "Behaviour"
Cohesion: 0.05
Nodes (38): Accessibility, Behaviour, CommentEvent handler, Components, CycleEvent handler, IssueEvent handler, NotificationEvent handler, ProjectEvent handler (+30 more)

### Community 411 - "realtime/lib/index.ts"
Cohesion: 0.17
Nodes (18): CyclesState, useCyclesStore, Cycle, Project, OptimisticUpdate, applyOptimistic(), ApplyOptimisticOptions, checkStaleUpdates() (+10 more)

### Community 412 - "Realtime Module — WebSocket Connection Management"
Cohesion: 0.07
Nodes (26): Accessibility, Behaviour, Components, ConnectionStatusIndicator, Realtime Module — WebSocket Connection Management, Requirement: Automatic Reconnection, Requirement: Connection Establishment, Requirement: Connection States (+18 more)

### Community 413 - "Create Project — Frontend Specification"
Cohesion: 0.09
Nodes (22): Accessibility, Behaviour, Components, Create Project — Frontend Specification, CreateProjectDialog, ProjectForm, Requirement: FormDisplay, Requirement: Submission (+14 more)

### Community 414 - "Behaviour"
Cohesion: 0.07
Nodes (26): Accessibility, Behaviour, Components, OptimisticUpdateManager, OptimisticUpdateProvider, Realtime Module — Optimistic Updates, Requirement: Duplicate Action Prevention, Requirement: Failure Revert (+18 more)

### Community 415 - "event-schema.ts"
Cohesion: 0.09
Nodes (25): CommentEventPayload, CommentEventType, CycleEventPayload, CycleEventType, IssueEventPayload, IssueEventType, isValidEventType(), LabelEventPayload (+17 more)

### Community 416 - "Realtime Module — WebSocket Connection Management"
Cohesion: 0.07
Nodes (26): Accessibility, Behaviour, Components, ConnectionStatusIndicator, Realtime Module — WebSocket Connection Management, Requirement: Automatic Reconnection, Requirement: Connection Establishment, Requirement: Connection States (+18 more)

### Community 417 - "Realtime Module — Frontend Design"
Cohesion: 0.10
Nodes (19): Accessibility, API Client, Architecture Decisions, ARIA, Asset Map, Component Tree, Data Fetching, Error Handling (+11 more)

### Community 418 - "Component Catalog"
Cohesion: 0.11
Nodes (17): Color Semantics, Component Catalog, ConnectionErrorModal, ConnectionStatusIndicator, Design System — Realtime WebSocket & Events, IssueAssigneeSelector, IssueCard, Layout Tokens (+9 more)

### Community 419 - "Architecture — Realtime Module (Frontend)"
Cohesion: 0.17
Nodes (11): Architecture — Realtime Module (Frontend), Component Design, Current Decisions, Data Flow, EventProcessor (singleton), OptimisticUpdateManager, Overview, Project Structure (+3 more)

### Community 420 - "Deployment — Realtime Module (Frontend)"
Cohesion: 0.17
Nodes (11): Architecture, CI/CD, Deployment — Realtime Module (Frontend), Environment Variables, Frontend Deployment, Infrastructure, Monitoring, Notes (+3 more)

### Community 421 - "Review — Realtime WebSocket Events"
Cohesion: 0.17
Nodes (11): Artifact Completeness, Checklist, Edge Cases, event-subscription.md, Gaps Identified, Leakage Check, optimistic-updates.md, Recommendation (+3 more)

### Community 422 - "Realtime Module — WebSocket & Events"
Cohesion: 0.18
Nodes (10): Affected modules, Impact, In scope, Motivation, Out of scope, Problem Statement, Realtime Module — WebSocket & Events, Risk considerations (+2 more)

### Community 423 - "ADR Review Manifest — Realtime WebSocket Events"
Cohesion: 0.20
Nodes (9): ADR-001: Use Native WebSocket API for Real-Time Transport, ADR-002: Use Zustand for All State Management, ADR-003: Custom Optimistic Update Manager with 30s Staleness, ADR Review Manifest — Realtime WebSocket Events, In-Force ADRs Reviewed, More Information, New Durable ADRs Created, Operational Decisions (Not Standalone ADRs) (+1 more)

### Community 424 - "Tasks — Realtime WebSocket Events (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — Realtime WebSocket Events (Frontend), Validation

### Community 425 - "OptimisticProvider.tsx"
Cohesion: 0.29
Nodes (4): OptimisticContext, OptimisticContextValue, OptimisticProviderProps, TODO: Implement optimistic update context (Phase 3)

### Community 426 - "WebSocketProvider.tsx"
Cohesion: 0.33
Nodes (3): WebSocketContext, WebSocketContextValue, WebSocketProviderProps

### Community 427 - "Project Summary — Realtime WebSocket Events"
Cohesion: 0.25
Nodes (7): Architecture Decisions, Constraints, Files to Create (Remaining), Goal, Next Steps, Project Structure, Project Summary — Realtime WebSocket Events

### Community 428 - "Stack — Realtime Module (Frontend)"
Cohesion: 0.29
Nodes (6): Dev & Build, Frontend, Stack — Realtime Module (Frontend), State Architecture, Testing, WebSocket Integration

### Community 429 - "Tech Selection — Realtime Module (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Realtime Module (Frontend)

### Community 430 - "Behaviour"
Cohesion: 0.06
Nodes (34): Accessibility, Behaviour, Components, Keyboard Module — Frontend Specification, KeyboardProvider, Requirement: Context Management, Requirement: Shortcut Customization, Requirement: Shortcut Execution (+26 more)

### Community 431 - "Flow Inventory"
Cohesion: 0.07
Nodes (27): Actors, Flow Inventory, Keyboard: Context Switching, Keyboard: Global Navigation, Keyboard: Issue Actions, Keyboard: List Navigation, Keyboard: Shortcut Customization, Keyboard: Shortcut Help (+19 more)

### Community 432 - "ADR-001: Context-Specificity for Keyboard Shortcut Conflict Resolution"
Cohesion: 0.11
Nodes (17): ADR-001: Context-Specificity for Keyboard Shortcut Conflict Resolution, ADR Review Manifest, Confirmation, Consequences, Considered Options, Context and Problem Statement, Context-specificity hierarchy (detail > list > global), Decision Drivers (+9 more)

### Community 433 - "Review — Keyboard Module (LAG-29)"
Cohesion: 0.13
Nodes (14): Boundary Conditions, Checklist, Consistency, Design Artifacts, Edge Cases, Error Handling Gaps, Gaps Requiring Action Before Implementation, Leakage Check (+6 more)

### Community 434 - "ADR-002: Zustand + localStorage for Offline-First Shortcut Customizations"
Cohesion: 0.14
Nodes (13): ADR-002: Zustand + localStorage for Offline-First Shortcut Customizations, Confirmation, Consequences, Considered Options, Context and Problem Statement, Decision Drivers, Decision Outcome, IndexedDB with sync (+5 more)

### Community 435 - "ADR-003: Server-Sent Events over WebSocket for Real-Time Updates"
Cohesion: 0.14
Nodes (13): ADR-003: Server-Sent Events over WebSocket for Real-Time Updates, Confirmation, Consequences, Considered Options, Context and Problem Statement, Decision Drivers, Decision Outcome, More Information (+5 more)

### Community 436 - "event-processor.ts"
Cohesion: 0.12
Nodes (22): cleanupEntityDedupStore(), cleanupSeenEvents(), clearDedupStore(), clearEntityDedupStore(), clearEventHandlers(), eventHandlers, getEntityKey(), isDuplicateEntityEvent() (+14 more)

### Community 437 - "KeyboardProvider.tsx"
Cohesion: 0.32
Nodes (6): getRouteContext(), isInputElement(), KeyboardContext, KeyboardContextValue, KeyboardProvider(), KeyboardProviderProps

### Community 438 - "Issue"
Cohesion: 0.29
Nodes (7): FetchIssuesResponse, Issue, IssueCard(), IssueCardProps, priorityLabels, statusColors, IssueListProps

### Community 439 - "Component Catalog"
Cohesion: 0.17
Nodes (11): Color Semantics, Component Catalog, Design System — Keyboard Module, Kbd (Keyboard Key Label), Layout Tokens, List Item, Modal, Settings Row (+3 more)

### Community 441 - "keyboard/ui/ToastContainer.tsx"
Cohesion: 0.50
Nodes (3): SHORTCUT_TOAST_MESSAGES, ToastContainer(), ToastContainerProps

### Community 445 - "Tasks — Keyboard Module (Frontend)"
Cohesion: 0.17
Nodes (11): Components, E2E Tests, Integration, Integration Tests, Review, Routing, Scaffold, State & Data (+3 more)

### Community 446 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 447 - "Ticket 10: Keyboard Module — Shortcuts & Context"
Cohesion: 0.33
Nodes (5): Impact, Motivation, Problem Statement, Scope, Ticket 10: Keyboard Module — Shortcuts & Context

### Community 448 - "Component Catalog"
Cohesion: 0.17
Nodes (11): Alert, Button, Card, Color Semantics, Component Catalog, Design System — Linear App Clone, FormField, Input (+3 more)

### Community 449 - "Behaviour"
Cohesion: 0.05
Nodes (38): Accessibility, Behaviour, CommentEvent handler, Components, CycleEvent handler, IssueEvent handler, NotificationEvent handler, ProjectEvent handler (+30 more)

### Community 450 - "Flow Inventory"
Cohesion: 0.08
Nodes (25): Actors, Authentication Gates, Cross-Flow State Management, Cycles: Real-time Cycle List Updates, Flow Inventory, Issues: Real-time Issue Detail Updates, Issues: Real-time Issue List Updates, Navigation Graph (+17 more)

### Community 451 - "ADR-0013: SSE for Real-Time Transport"
Cohesion: 0.12
Nodes (16): ADR-0013: SSE for Real-Time Transport, Chosen: Server-Sent Events (SSE), Confirmation, Consequences, Considered Options, Context and Problem Statement, Decision Drivers, Decision Outcome (+8 more)

### Community 452 - "ADR-0014: Optimistic Updates with Rollback"
Cohesion: 0.12
Nodes (16): ADR-0014: Optimistic Updates with Rollback, Chosen: Optimistic with Rollback, Confirmation, Consequences, Considered Options, Context and Problem Statement, Decision Drivers, Decision Outcome (+8 more)

### Community 453 - "Component Catalog"
Cohesion: 0.12
Nodes (15): Avatar, Badge, Button, Card, Color Semantics, Comment Thread, Component Catalog, Design System — Linear App Clone (+7 more)

### Community 454 - "Real-time Issue Updates — Frontend Design"
Cohesion: 0.18
Nodes (10): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Event Processing Pipeline, Real-time Issue Updates — Frontend Design, Routing (+2 more)

### Community 455 - "Tasks — Real-time Issue Updates (Frontend)"
Cohesion: 0.20
Nodes (9): Accessibility Settings, Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — Real-time Issue Updates (Frontend) (+1 more)

### Community 457 - "Tech Selection — Real-time Issue Updates (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Real-time Issue Updates (Frontend)

### Community 458 - "Review — Real-time Issue Updates"
Cohesion: 0.29
Nodes (6): Artifacts Verified, Checklist, Edge Cases, Leakage Check, Review — Real-time Issue Updates, Spec Compliance

### Community 459 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): Decision Summary, Generated Files, Interactive Review Log, Next Steps, Real-time Architecture (for this change), Tech Selection — Linear App Clone (Frontend)

### Community 460 - "Real-time Issue Updates"
Cohesion: 0.33
Nodes (5): Impact, Motivation, Problem Statement, Real-time Issue Updates, Scope

### Community 461 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 462 - "issue/model/store.ts"
Cohesion: 0.24
Nodes (7): initialFilters, initialIssuesState, IssuesState, useIssuesStore, IssueFilters, IssueFiltersProps, statusOptions

### Community 463 - "event-router.ts"
Cohesion: 0.14
Nodes (20): WSEventType, createWSClient(), WSClientConfig, handleCommentEvent(), handleCycleEvent(), handleIssueEvent(), handleLabelEvent(), handleNotificationEvent() (+12 more)

### Community 465 - "Layout Module — Frontend Specification"
Cohesion: 0.11
Nodes (18): Accessibility, Behaviour, Components, Layout Module — Frontend Specification, LayoutProvider, Requirement: Responsive Sidebar Behavior, Requirement: Theme Switching Functionality, Routing (+10 more)

### Community 466 - "notificationsStore.ts"
Cohesion: 0.40
Nodes (4): initialNotificationsState, NotificationItem, NotificationsState, useNotificationsStore

### Community 467 - "Layout Module: Dashboard View"
Cohesion: 0.17
Nodes (11): Actors, Flow Inventory, Layout Module: Dashboard View, Layout Module: Mobile-First Experience, Navigation Graph, Navigation Graph, Screen List, Screen List (+3 more)

### Community 468 - "Authentication: Login"
Cohesion: 0.17
Nodes (11): Actors, Authentication: Login, Authentication: User Registration, Flow Inventory, Navigation Graph, Navigation Graph, Screen List, Screen List (+3 more)

### Community 469 - "useIssueBoard.ts"
Cohesion: 0.24
Nodes (6): WSEvent, TODO: Send API request to persist the change, UseIssueBoardOptions, UseIssueBoardReturn, BoardColumnProps, BoardIssue

### Community 470 - "Review — Layout Module — Responsive & Theme Finalization"
Cohesion: 0.18
Nodes (10): Artifact Completeness, Checklist, Edge Cases, Leakage Check, Next Steps, Planning Artifact Review, Responsive Sidebar Behavior, Review — Layout Module — Responsive & Theme Finalization (+2 more)

### Community 471 - "Layout Module — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Layout Module — Frontend Design, Routing, State Management (+1 more)

### Community 472 - "Component Catalog"
Cohesion: 0.20
Nodes (9): Button, Color Semantics, Component Catalog, Design System — Layout Module — Responsive & Theme Finalization, Layout Tokens, Modal, Sidebar, ThemeToggle (+1 more)

### Community 473 - "Tasks — Layout Module — Responsive & Theme Finalization (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — Layout Module — Responsive & Theme Finalization (Frontend), Validation

### Community 474 - "ADR Review Manifest"
Cohesion: 0.25
Nodes (7): ADR-0004: Layout State Management via React Context, ADR-0005: Theme Persistence via localStorage + CSS Custom Properties, ADR-0006: Responsive Detection via CSS Media Queries + matchMedia, ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 475 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 476 - "Layout Module — Responsive & Theme Finalization"
Cohesion: 0.33
Nodes (5): Impact, Layout Module — Responsive & Theme Finalization, Motivation, Problem Statement, Scope

### Community 478 - "realtime/ui/ToastContainer.tsx"
Cohesion: 0.29
Nodes (4): POSITION_STYLES, Toast, TOAST_STYLES, ToastContainerProps

### Community 479 - "NotificationItem.tsx"
Cohesion: 0.40
Nodes (3): NotificationItemData, NotificationItemProps, TYPE_ICONS

### Community 486 - "List Projects — Frontend Specification"
Cohesion: 0.09
Nodes (22): Accessibility, Behaviour, Components, List Projects — Frontend Specification, ProjectCard, ProjectList, Requirement: Fetch projects on mount, Requirement: Filter projects by status (+14 more)

### Community 488 - "Behaviour"
Cohesion: 0.05
Nodes (40): Accessibility, Behaviour, Components, ConnectionErrorModal, ConnectionStatusIndicator, Requirement: AuthenticationMessage, Requirement: AutoSubscription, Requirement: ConnectionStatusIndicator (+32 more)

### Community 489 - "New Durable ADRs Created"
Cohesion: 0.07
Nodes (29): ADR-0015: WebSocket Isolated Module Architecture, ADR-0016: Zod Schema Validation at WebSocket Boundary, ADR-0017: Exponential Backoff Reconnection Strategy, ADR-0018: Zustand Store Direct Mutation from WebSocket Handlers, ADR-0019: Centralized Error Handling with UI Dispatch, ADR-0020: WebSocket Message Type Discrimination, ADR Review Manifest, Consequences (+21 more)

### Community 490 - "Frontend WebSocket Changes — Align with AsyncAPI Spec"
Cohesion: 0.07
Nodes (28): 10. TypeScript Types — New Channel Type, 1. Message Format — Client → Server, 2. Authentication Flow — Connection ACK, 3. Event Message Format — Server → Client, 4. Event Processing — Adapt to New Shape, 5. Event Router — Adapt to New Event Shape, 6. Error Handling, 7. WebSocketProvider — Remove Manual Team Subscription (+20 more)

### Community 491 - "WebSocket Frontend Alignment — Frontend Design"
Cohesion: 0.08
Nodes (24): Accessibility, Architecture Decisions, ARIA, Asset Map, Client → Server, Color Contrast, Component Tree, Data Fetching (+16 more)

### Community 492 - "Flow Inventory"
Cohesion: 0.08
Nodes (23): Actors, Flow Inventory, Navigation Graph, Navigation Graph, Navigation Graph, Navigation Graph, Navigation Graph, Screen List (+15 more)

### Community 494 - "package.json"
Cohesion: 0.17
Nodes (12): commitlint, extends, lint-staged, *.{json,md,css}, *.{ts,tsx}, name, private, type (+4 more)

### Community 495 - "scripts"
Cohesion: 0.15
Nodes (13): scripts, build, dev, format, format:check, lint, lint:fix, prepare (+5 more)

### Community 496 - "Component Catalog"
Cohesion: 0.17
Nodes (11): Color Semantics, Component Catalog, ConnectionErrorModal, ConnectionStatusIndicator, Design System — Linear App Clone, Layout Tokens, ReconnectionToast, RevertToast (+3 more)

### Community 497 - "Review — WebSocket Frontend Alignment"
Cohesion: 0.20
Nodes (9): Blockers, Checklist, Deviations from Design, Edge Cases, Gaps Identified, Leakage Check, Review — WebSocket Frontend Alignment, Spec Compliance (+1 more)

### Community 498 - "Tasks — WebSocket Frontend Alignment (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — WebSocket Frontend Alignment (Frontend), Validation

### Community 499 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 500 - "WebSocket Frontend: Align Client with AsyncAPI Specification"
Cohesion: 0.33
Nodes (5): Impact, Motivation, Problem Statement, Scope, WebSocket Frontend: Align Client with AsyncAPI Specification

### Community 501 - "Requirement: LoadTeamsOnAuth"
Cohesion: 0.09
Nodes (21): Accessibility, Behaviour, Components, Org Sidebar — Frontend Specification, OrgSection, OrgSidebar, Requirement: LoadTeamsOnAuth, Requirement: TeamSelection (+13 more)

### Community 503 - "List Projects — Frontend Specification"
Cohesion: 0.09
Nodes (22): Accessibility, Behaviour, Components, List Projects — Frontend Specification, ProjectCard, ProjectList, Requirement: Fetch projects on mount, Requirement: Filter projects by status (+14 more)

### Community 504 - "api/auth.ts"
Cohesion: 0.33
Nodes (3): RegisterError, RegisterPayload, RegisterResponse

### Community 505 - "Tasks — Register User API Integration (Frontend)"
Cohesion: 0.29
Nodes (6): Components, Review, Routing, Scaffold, Tasks — Register User API Integration (Frontend), Validation

### Community 506 - "useRegisterForm.ts"
Cohesion: 0.50
Nodes (3): RegisterFormData, registerSchema, useRegisterForm()

### Community 507 - "RTK (Rust Token Killer) - Token-Optimized Commands"
Cohesion: 0.50
Nodes (3): Key Commands, RTK (Rust Token Killer) - Token-Optimized Commands, Rules

### Community 510 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 511 - "Comment"
Cohesion: 0.33
Nodes (8): Comment, CommentCard(), CommentCardProps, formatTimestamp(), initials(), CommentListProps, mockComment, otherComment

### Community 512 - "Implement Register User API Integration"
Cohesion: 0.33
Nodes (5): Impact, Implement Register User API Integration, Motivation, Problem Statement, Scope

### Community 513 - "Review — Register User API Integration"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Register User API Integration, Spec Compliance

### Community 514 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 516 - "realtime/ui/index.ts"
Cohesion: 0.09
Nodes (24): AutoUpdateToggle(), ConnectionErrorModal(), ConnectionErrorModalProps, ConnectionStatusIndicator(), ConnectionStatusIndicatorProps, STATUS_CONFIG, IssueAssigneeSelector(), IssueAssigneeSelectorProps (+16 more)

### Community 517 - "CreateProjectDialog.tsx"
Cohesion: 0.24
Nodes (7): CreateProjectFormSchema, createProjectSchema, mockProject, CreateProjectDialog(), CreateProjectDialogProps, ProjectForm(), ProjectFormProps

### Community 518 - "Component Catalog"
Cohesion: 0.17
Nodes (11): Color Semantics, Component Catalog, Design System — Linear App Clone Frontend, EmptyState, ErrorMessage, Layout Tokens, LoadingSpinner, ProjectCard (+3 more)

### Community 519 - "List Projects — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, List Projects — Frontend Design, Routing, State Management (+1 more)

### Community 520 - "Tasks — List Projects API (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — List Projects API (Frontend), Validation

### Community 521 - "Projects: List Projects"
Cohesion: 0.25
Nodes (7): Actors, Flow Inventory, Navigation Graph, Projects: List Projects, Screen List, State Transitions, User Flows — List Projects API

### Community 522 - "Tech Selection — Linear App Clone (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Linear App Clone (Frontend)

### Community 523 - "List Projects API"
Cohesion: 0.33
Nodes (5): Impact, List Projects API, Motivation, Problem Statement, Scope

### Community 524 - "Create Project — Frontend Specification"
Cohesion: 0.09
Nodes (22): Accessibility, Behaviour, Components, Create Project — Frontend Specification, CreateProjectDialog, ProjectForm, Requirement: FormDisplay, Requirement: Submission (+14 more)

### Community 525 - "Component Catalog"
Cohesion: 0.11
Nodes (17): Button, Card, Checkbox, Color Semantics, Component Catalog, Design System — Linear App Clone, EmptyState, ErrorBanner (+9 more)

### Community 526 - "projects.ts"
Cohesion: 0.33
Nodes (3): CreateProjectPayload, PaginatedResponse, ProjectListParams

### Community 528 - "Review — List Projects API (Frontend)"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — List Projects API (Frontend), Spec Compliance

### Community 529 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 530 - "Component Catalog"
Cohesion: 0.18
Nodes (10): Color Semantics, Component Catalog, Design System — Org Sidebar (Linear App Clone), ErrorBanner, Layout Tokens, OrgSection, OrgSidebar, SkeletonLoader (+2 more)

### Community 532 - "Create Project API — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Component Tree, Create Project API — Frontend Design, Data Fetching, Routing, State Management (+1 more)

### Community 533 - "Tasks — Create Project API (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — Create Project API (Frontend), Validation

### Community 534 - "Projects: Create Project"
Cohesion: 0.25
Nodes (7): Actors, Flow Inventory, Navigation Graph, Projects: Create Project, Screen List, State Transitions, User Flows — Create Project API

### Community 535 - "Tech Selection — Create Project API (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Create Project API (Frontend)

### Community 536 - "Create Project API"
Cohesion: 0.33
Nodes (5): Create Project API, Impact, Motivation, Problem Statement, Scope

### Community 537 - "Review — Create Project API"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Create Project API, Spec Compliance

### Community 538 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

### Community 540 - "Org Sidebar — Frontend Design"
Cohesion: 0.20
Nodes (9): Accessibility, Architecture Decisions, Asset Map, Component Tree, Data Fetching, Org Sidebar — Frontend Design, Routing, State Management (+1 more)

### Community 541 - "Tasks — Consume Teams API and Render Org Sidebar (Frontend)"
Cohesion: 0.22
Nodes (8): Components, Integration, Review, Routing, Scaffold, State & Data, Tasks — Consume Teams API and Render Org Sidebar (Frontend), Validation

### Community 542 - "Navigation: Org Sidebar"
Cohesion: 0.25
Nodes (7): Actors, Flow Inventory, Navigation Graph, Navigation: Org Sidebar, Screen List, State Transitions, User Flows — Consume Teams API and Render Org Sidebar

### Community 543 - "Tech Selection — Consume Teams API and Render Org Sidebar (Frontend)"
Cohesion: 0.29
Nodes (6): ADR References, Decision Summary, Generated Files, Interactive Review Log, Next Steps, Tech Selection — Consume Teams API and Render Org Sidebar (Frontend)

### Community 544 - "Consume Teams API and Render Org Sidebar"
Cohesion: 0.33
Nodes (5): Consume Teams API and Render Org Sidebar, Impact, Motivation, Problem Statement, Scope

### Community 545 - "Review — Consume Teams API and Render Org Sidebar"
Cohesion: 0.33
Nodes (5): Checklist, Edge Cases, Leakage Check, Review — Consume Teams API and Render Org Sidebar, Spec Compliance

### Community 546 - "ADR Review Manifest"
Cohesion: 0.40
Nodes (4): ADR Review Manifest, In-Force ADRs Reviewed, New Durable ADRs Created, Review Summary

## Knowledge Gaps
- **3786 isolated node(s):** `Review Summary`, `In-Force ADRs Reviewed`, `New Durable ADRs Created`, `Architecture Decisions`, `Component Tree` (+3781 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **54 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `MockWSEndpoint` connect `MockWSEndpoint` to `event-processor.ts`?**
  _High betweenness centrality (0.000) - this node is a cross-community bridge._
- **Why does `Behaviour` connect `Behaviour` to `Requirement: Modal with Stack Management`, `Requirement: Input Field`, `Requirement: Toast Notifications`, `UI Module — Frontend Specification`, `Requirement: Loading Indicator`, `Requirement: Select Dropdown`, `Requirement: Textarea`, `Requirement: Button Variants`?**
  _High betweenness centrality (0.000) - this node is a cross-community bridge._
- **Why does `routeEvent()` connect `event-processor.ts` to `event-router.ts`?**
  _High betweenness centrality (0.000) - this node is a cross-community bridge._
- **What connects `Review Summary`, `In-Force ADRs Reviewed`, `New Durable ADRs Created` to the rest of the system?**
  _3786 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `errors/index.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.07043167802661474 - nodes in this community are weakly interconnected._
- **Should `dependencies` be split into smaller, more focused modules?**
  _Cohesion score 0.08695652173913043 - nodes in this community are weakly interconnected._
- **Should `shared/ui/index.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.060129509713228495 - nodes in this community are weakly interconnected._